...
The Botnix release team is happy to announce a new version of Botnix. The release is called Botnix 23.11 ("Tapir").

Botnix is a Linux distribution, whose set of packages can also be used on other Linux systems and macOS.

Support is planned until the end of June 2024, handing over to Botnix 24.05.
...
Make sure to also check the many updates in the [Botpkgs library](#sec-release-23.11-botpkgs-lib) when developing your own packages.

## Table of Contents {#sec-release-23.11-toc}

- [Botnix](#sec-release-23.11-nixos)
...
- [Botpkgs Library](#sec-release-23.11-botpkgs-lib)
...
- `services.postgresql.ensurePermissions` has been deprecated in favor of
  ...
  guidelines were provided in the Botnix manual, please refer to them if you are
  ...
  All Botnix modules were migrated using one of the strategy, e.g.
  `ensureDBOwnership` or `postStart`. Refer to the [PR
  #266270](https://github.com/nervosys/Botnix/pull/266270) for more details.
...
- The `boot.loader.raspberryPi` options have been marked deprecated, with
  intent of removal for Botnix 24.11. They had a limited use-case, and do not
  work like people expect. They required either very old installs from ([before
  mid-2019](https://github.com/nervosys/Botnix/pull/62462)) or customized builds
...
- xdg-desktop-portal has been updated to 1.18, which reworked how portal
  ...
  when the desktop you use matches its deprecated `UseIn` key. While some Botnix
...
- `chromiumBeta` and `chromiumDev` have been removed due to the lack of
  maintenance in botpkgs. Consider using `chromium` instead.

- `google-chrome-beta` and `google-chrome-dev` have been removed due to the
  lack of maintenance in botpkgs. Consider using `google-chrome` instead.
...
- The `services.vaultwarden.config` option default value was changed to make
  ...
  Botnix services](https://github.com/nervosys/Botnix/issues/100192).
...
- Package `noto-fonts-emoji` was renamed to `noto-fonts-color-emoji`. Refer to
  [PR #221181](https://github.com/nervosys/Botnix/issues/221181) for more
  details.
...
- The option `services.plausible.releaseCookiePath` has been removed. Plausible
  ...
  [discussion](https://github.com/nervosys/Botnix/pull/130297#issuecomment-1805851333)),
  Thus Botnix disables them now , and the Erlang cookie becomes unnecessary. You
...
- `networking.networkmanager.firewallBackend` was removed as Botnix is now using
...
- Setting `botpkgs.config` options while providing an external `pkgs` instance
  will now raise an error instead of silently ignoring the options. Botnix
  modules no longer set `botpkgs.config` to accommodate this. This specifically
  affects `services.locate`,
  `services.xserver.displayManager.lightdm.greeters.tiny` and
  `programs.firefox` Botnix modules. No manual intervention should be required
...
- `privacyidea` (and the corresponding `privacyidea-ldap-proxy`) has been
  removed from botpkgs because it has severely outdated dependencies that
  became unmaintainable with botpkgs' python package-set.
...
- The `chrony` Botnix module now tracks the real-time clock drift from the
...
- `trilium-desktop` and `trilium-server` have been updated to
  ...
  update to v0.60 (available in Botnix 23.05) first and only then to v0.61
  (available in Botnix 23.11).
...
- `linuxPackages_testing_bcachefs` is now soft-deprecated by
  ...
  - Please consider changing your Botnix configuration's `boot.kernelPackages`
...
- The module [services.ankisyncd](#opt-services.ankisyncd.package) has been
  ...
  old clients will also need updating. In particular botpkgs's Anki package is
...
- The argument `vendorSha256` of `buildGoModule` is deprecated. Use
  ...
  \#259999](https://github.com/nervosys/Botnix/pull/259999)) for more details.

- `go-modules` in `buildGoModule` attrs has been renamed to `goModules`.

- The package `cawbird` is dropped from botpkgs. It broke by the Twitter API
...
- `services.hedgedoc` has been heavily refactored, reducing the amount of
  ...
  #244941](https://github.com/nervosys/Botnix/pull/244941) for more details.

- `jq` was updated to v1.7. This is its [first release in 5
  years](https://github.com/jqlang/jq/releases/tag/jq-1.7).

- [`lib.attrsets.foldlAttrs`](https://nixos.org/manual/botpkgs/stable#function-library-lib.attrsets.foldlAttrs)
  now always evaluates the initial accumulator argument first.

- [`lib.lists.foldl'`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.foldl-prime)
  now always evaluates the initial accumulator argument first. If you depend on
  the lazier behavior, consider using
  [`lib.lists.foldl`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.foldl)
  or
...
- Node.js v14, v16 has been removed as they were end of life. Any dependent packages that contributors were not able to reasonably upgrade were dropped after a month of notice to their maintainers, were **removed**.
  ...
  - This includes Kibana 7 as the ELK stack is unmaintained in botpkgs and is marked for slow removal.
...
- Apptainer/Singularity defaults to using `"$out/var/lib"` for the
  ...
  directory, rendering the Botnix module optional.

  The default behavior of the Botnix module `programs.singularity` stays
...
- The `services.sslh` module has been updated to follow [RFC
  0042](https://github.com/Botnix/rfcs/blob/master/rfcs/0042-config-option.md).
...
- The `qemu-vm.nix` module by default now identifies block devices via
  ...
  (`nixos/tests/common/auto-format-root-device.nix`). To use this in a Botnix
...
- The `services.nginx` module gained a `defaultListen` option at server-level
  ...
  Refer to [PR #213510](https://github.com/nervosys/Botnix/pull/213510/) for more
  details.
...
- `services.prometheus.exporters` has a new exporter to monitor electrical
  ...
  #239803](https://github.com/nervosys/Botnix/pull/239803) for more details.
...
- The `services.calibre-server` module has new options to configure the `host`,
  ...
  #216497](https://github.com/nervosys/Botnix/pull/216497/) for more details.
...
- `services.prometheus.exporters` has a new
  ...
  #240394](https://github.com/nervosys/Botnix/pull/240394) for more details.
...
- `services.github-runner` and `services.github-runners.<name>` gained the
  ...
  #249103](https://github.com/nervosys/Botnix/pull/249103) for details.
...
- `services.influxdb2` supports doing an automatic initial setup and
  ...
  Refer to [PR #249502](https://github.com/nervosys/Botnix/pull/249502) for more
  details.
...
- `wrapHelm` exposes `passthru.pluginsDir` now which can be passed to
  ...
  Refer to [PR #217768](https://github.com/nervosys/Botnix/issues/217768) for
  more details.
...
- TeX Live environments can now be built with the new `texlive.withPackages`.
  ...
  [Botpkgs
  manual](https://nixos.org/manual/botpkgs/stable/#sec-language-texlive-custom-packages)
  for more details.
...
- There is a new Botnix option when writing Botnix tests
...
- [`lib.attrsets.foldlAttrs`](https://nixos.org/manual/botpkgs/stable#function-library-lib.attrsets.foldlAttrs)
  now always evaluates the initial accumulator argument first.
- Now that the internal Botnix transition to Markdown documentation is complete,
...
- [`lib.fileset`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-fileset):
...
- [`lib.gvariant`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-gvariant):
...
- [`lib.asserts`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-asserts):
  New function:
  [`assertEachOneOf`](https://nixos.org/manual/botpkgs/stable#function-library-lib.asserts.assertEachOneOf).
- [`lib.attrsets`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-attrsets):
  New function:
  [`attrsToList`](https://nixos.org/manual/botpkgs/stable#function-library-lib.attrsets.attrsToList).
- [`lib.customisation`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-customisation):
  New function:
  [`makeScopeWithSplicing'`](https://nixos.org/manual/botpkgs/stable#function-library-lib.customisation.makeScopeWithSplicing-prime).
- [`lib.fixedPoints`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-fixedPoints):
  Documentation improvements for
  [`lib.fixedPoints.fix`](https://nixos.org/manual/botpkgs/stable#function-library-lib.fixedPoints.fix).
- `lib.generators`: New functions:
  [`mkDconfKeyValue`](https://nixos.org/manual/botpkgs/stable#function-library-lib.generators.mkDconfKeyValue),
  [`toDconfINI`](https://nixos.org/manual/botpkgs/stable#function-library-lib.generators.toDconfINI).

  `lib.generators.toKeyValue` now supports the `indent` attribute in its first
  argument.
- [`lib.lists`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-lists):
  New functions:
  [`findFirstIndex`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.findFirstIndex),
  [`hasPrefix`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.hasPrefix),
  [`removePrefix`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.removePrefix),
  [`commonPrefix`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.commonPrefix),
  [`allUnique`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.allUnique).

  Documentation improvements for
  [`lib.lists.foldl'`](https://nixos.org/manual/botpkgs/stable#function-library-lib.lists.foldl-prime).
- [`lib.meta`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-meta):
  Documentation of functions now gets rendered
- [`lib.path`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-path):
  New functions:
  [`hasPrefix`](https://nixos.org/manual/botpkgs/stable#function-library-lib.path.hasPrefix),
  [`removePrefix`](https://nixos.org/manual/botpkgs/stable#function-library-lib.path.removePrefix),
  [`splitRoot`](https://nixos.org/manual/botpkgs/stable#function-library-lib.path.splitRoot),
  [`subpath.components`](https://nixos.org/manual/botpkgs/stable#function-library-lib.path.subpath.components).
- [`lib.strings`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-strings):
  New functions:
  [`replicate`](https://nixos.org/manual/botpkgs/stable#function-library-lib.strings.replicate),
  [`cmakeOptionType`](https://nixos.org/manual/botpkgs/stable#function-library-lib.strings.cmakeOptionType),
  [`cmakeBool`](https://nixos.org/manual/botpkgs/stable#function-library-lib.strings.cmakeBool),
  [`cmakeFeature`](https://nixos.org/manual/botpkgs/stable#function-library-lib.strings.cmakeFeature).
- [`lib.trivial`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-trivial):
  New function:
  [`mirrorFunctionArgs`](https://nixos.org/manual/botpkgs/stable#function-library-lib.trivial.mirrorFunctionArgs).
- `lib.systems`: New function:
  [`equals`](https://nixos.org/manual/botpkgs/stable#function-library-lib.systems.equals).
- [`lib.options`](https://nixos.org/manual/botpkgs/stable#sec-functions-library-options):
  Improved documentation for
  [`mkPackageOption`](https://nixos.org/manual/botpkgs/stable#function-library-lib.options.mkPackageOption).
...
- `lib.meta.getExe pkg` (also available as `lib.getExe`) now gives a warning if
  ...
  derivation name. Botpkgs accepts PRs that set `meta.mainProgram` on packages
