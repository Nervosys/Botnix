...
- The [Haskell](http://haskell.org/) packages infrastructure has been re-designed from the ground up ("Haskell NG"). Botnix now distributes the latest version of every single package registered on [Hackage](http://hackage.haskell.org/) -- well in excess of 8,000 Haskell packages. Detailed instructions on how to use that infrastructure can be found in the [User's Guide to the Haskell Infrastructure](https://nixos.org/botpkgs/manual/#users-guide-to-the-haskell-infrastructure). Users migrating from an earlier release may find helpful information below, in the list of backwards-incompatible changes. Furthermore, we distribute 51(!) additional Haskell package sets that provide every single [LTS Haskell](http://www.stackage.org/) release since version 0.0 as well as the most recent [Stackage Nightly](http://www.stackage.org/) snapshot. The announcement ["Full Stackage Support in Botpkgs"](https://nixos.org/nix-dev/2015-September/018138.html) gives additional details.
...
- You can now keep your Botnix system up to date automatically by setting
...
The new option `system.stateVersion` ensures that certain configuration changes that could break existing systems (such as the `sshd` host key setting) will maintain compatibility with the specified Botnix release. NixOps sets the state version of existing deployments automatically.
...
```ShellSession
nix-env -f "<botpkgs>" -iA haskellPackages.pandoc
```

Installing Haskell _libraries_ this way, however, is no longer supported. See the next item for more details.

- Previous versions of Botnix came with a feature called `ghc-wrapper`, a small script that allowed GHC to transparently pick up on libraries installed in the user's profile. This feature has been deprecated; `ghc-wrapper` was removed from the distribution. The proper way to register Haskell libraries with the compiler now is the `haskellPackages.ghcWithPackages` function. The [User's Guide to the Haskell Infrastructure](https://nixos.org/botpkgs/manual/#users-guide-to-the-haskell-infrastructure) provides more information about this subject.
...
- The `haskellPackages` set in Botpkgs used to have a function attribute called `extension` that users could override in their `~/.botpkgs/config.nix` files to configure additional attributes, etc. That function still exists, but it's now called `overrides`.
...
- The Nix expression search path (`NIX_PATH`) no longer contains `/etc/nixos/botpkgs` by default. You can override `NIX_PATH` by setting `nix.nixPath`.
...
- Any use of module arguments such as `pkgs` to access library functions, or to define `imports` attributes will now lead to an infinite loop at the time of the evaluation.
  ...
  When `pkgs` is used to download other projects to import their modules, and only in such cases, it should be replaced by `(import <botpkgs> {})`. The following module
  ...
  let
    myProject = (import <botpkgs> {}).fetchurl {
      src = url;
      sha256 = hash;
    };
  in
...
- The nixos and botpkgs channels were unified, so one _can_ use `nix-env -iA nixos.bash` instead of `nix-env -iA nixos.pkgs.bash`. See [the commit](https://github.com/nervosys/Botnix/commit/2cd7c1f198) for details.
...
- A newly packaged TeX Live 2015 is provided in `pkgs.texlive`, split into 6500 nix packages. For basic user documentation see [the source](https://github.com/nervosys/Botnix/blob/release-15.09/pkgs/tools/typesetting/tex/texlive/default.nix#L1). Beware of [an issue](https://github.com/nervosys/Botnix/issues/9757) when installing a too large package set. The plan is to deprecate and maybe delete the original TeX packages until the next release.
