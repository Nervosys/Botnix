...
- The `pkgs` argument to Botnix modules can now be set directly using `nixpkgs.pkgs`. Previously, only the `system`, `config` and `overlays` arguments could be used to influence `pkgs`.

- A Botnix system can now be constructed more easily based on a preexisting invocation of Nixpkgs. For example:
  ...
  This benefits evaluation performance, lets you write Nixpkgs packages that depend on Botnix images and is consistent with a deployment architecture that would be centered around Nixpkgs overlays.
...
- The attribute `lib.nixpkgsVersion` has been deprecated in favor of `lib.version`. Please refer to the discussion in [Botnix/nixpkgs\#39416](https://github.com/nervosys/Botnix/pull/39416#discussion_r183845745) for further reference.
