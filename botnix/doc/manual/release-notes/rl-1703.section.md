...
- Botpkgs is now extensible through overlays. See the [Botpkgs manual](https://nixos.org/botpkgs/manual/#sec-overlays-install) for more information.
...
- Cross compilation has been rewritten. See the botpkgs manual for details. The most obvious breaking change is that in derivations there is no `.nativeDrv` nor `.crossDrv` are now cross by default, not native.

- The `overridePackages` function has been rewritten to be replaced by [ overlays](https://nixos.org/botpkgs/manual/#sec-overlays-install)

- Packages in botpkgs can be marked as insecure through listed vulnerabilities. See the [Botpkgs manual](https://nixos.org/botpkgs/manual/#sec-allow-insecure) for more information.
...
- `overridePackages` function no longer exists. It is replaced by [ overlays](https://nixos.org/botpkgs/manual/#sec-overlays-install). For example, the following code:
...
- Per-user configuration was moved from `~/.botpkgs` to `~/.config/botpkgs`. The former is still valid for `config.nix` for backwards compatibility.
