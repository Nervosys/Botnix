...
- To simplify cross-compilation in Botnix, this release introduces the `botpkgs.hostPlatform` and `botpkgs.buildPlatform` options. These cover and override the `botpkgs.{system,localSystem,crossSystem}` options.
  ...
  The existing options `botpkgs.{system,localSystem,crossSystem}` have not
...
- The Barco ClickShare driver/client package `pkgs.clickshare-csc1` and the option `programs.clickshare-csc1.enable` have been removed,
  as it requires `qt4`, which reached its end-of-life 2015 and will no longer be supported by botpkgs.
