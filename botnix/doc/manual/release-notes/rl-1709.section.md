...
- Modules can now be disabled by using [ disabledModules](https://nixos.org/botpkgs/manual/#sec-replace-modules), allowing another to take it's place. This can be used to import a set of modules from another channel while keeping the rest of the system on a stable release.
...
- Botpkgs overlays may now be specified with a file as well as a directory. The value of `<botpkgs-overlays>` may be a file, and `~/.config/botpkgs/overlays.nix` can be used instead of the `~/.config/botpkgs/overlays` directory.

  See the overlays chapter of the Botpkgs manual for more details.
