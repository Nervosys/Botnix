...
- Botnix now installs a stub ELF loader that prints an informative error message when users attempt to run binaries not made for Botnix.
...
- It is now possible to have a completely perlless system (i.e. a system
  without perl). Previously, the Botnix activation depended on two perl scripts
...
- The `power.ups` module now generates `upsd.conf`, `upsd.users` and `upsmon.conf` automatically from a set of new configuration options. This breaks compatibility with existing `power.ups` setups where these files were created manually. Back up these files before upgrading Botnix.
...
- Invidious has changed its default database username from `kemal` to `invidious`. Setups involving an externally provisioned database (i.e. `services.invidious.database.createLocally == false`) should adjust their configuration accordingly. The old `kemal` user will not be removed automatically even when the database is provisioned automatically.(https://github.com/nervosys/Botnix/pull/265857)
...
- The package `optparse-bash` is now dropped due to upstream inactivity. Alternatives available in Botpkgs include [`argc`](https://github.com/sigoden/argc), [`argbash`](https://github.com/matejak/argbash), [`bashly`](https://github.com/DannyBen/bashly) and [`gum`](https://github.com/charmbracelet/gum), to name a few.
...
- Programs written in [Nim](https://nim-lang.org/) are built with libraries selected by lockfiles.
  ...
  See https://nixos.org/manual/botpkgs/unstable#nim for more information.
...
- [Portunus](https://github.com/majewsky/portunus) has been updated to major version 2.
  ...
  Support for weak password hashes will be removed in Botnix 24.11.
...
- `security.pam.enableSSHAgentAuth` was renamed to `security.pam.sshAgentAuth.enable` and an `authorizedKeysFiles`
  ...
  **which is insecure**: see [#31611](https://github.com/nervosys/Botnix/issues/31611).
...
- The `hardware.pulseaudio` module now sets permission of pulse user home directory to 755 when running in "systemWide" mode. It fixes [issue 114399](https://github.com/nervosys/Botnix/issues/114399).
