...
- The GNOME and Plasma installation CDs now use `pkgs.calamares` and `pkgs.calamares-nixos-extensions` to allow users to easily install and set up Botnix with a GUI.
...
- [nix-ld](https://github.com/Mic92/nix-ld), Run unpatched dynamic binaries on Botnix. Available as [programs.nix-ld](#opt-programs.nix-ld.enable).
...
- `lib.systems.supported` has been removed, as it was overengineered for determining the systems to support in the botpkgs flake. The list of systems exposed by the botpkgs flake can now be accessed as `lib.systems.flakeExposed`.
...
- In the PowerDNS Recursor module (`services.pdns-recursor`), default values of several IP address-related Botnix options have been updated to match the default upstream behavior.
...
- `fonts.fonts` no longer includes ancient bitmap fonts when both `config.services.xserver.enable` and `config.botpkgs.config.allowUnfree` are enabled.
...
- The Keycloak package (`pkgs.keycloak`) has been switched from the
  ...
  - `services.keycloak.extraConfig` has been removed in favor of the
    new [settings-style](https://github.com/Botnix/rfcs/blob/master/rfcs/0042-config-option.md)
...
- The MoinMoin wiki engine (`services.moinmoin`) has been removed, because Python 2 is being retired from botpkgs.
...
- Services in the `hadoop` module previously set `openFirewall` to true by default.
  ...
  `openFirewall = true;` to be added to to hadoop services when upgrading from Botnix 21.11.
...
- opensmtpd-extras is no longer build with python2 scripting support due to python2 deprecation in botpkgs
...
- MultiMC has been replaced with the fork PrismLauncher due to upstream
  ...
  packages like the one contained in Botpkgs. This change affects the data
...
`$out/libexec/terraform-providers/<registry>/<owner>/<name>/<version>/<os>_<arch>/terraform-provider-<name>_v<version>` (which mkProvider does).
  ...
  This breaks back-compat so it's not possible to mix-and-match with previous versions of botpkgs. In exchange, it now becomes possible to use the providers from [botpkgs-terraform-providers-bin](https://github.com/numtide/botpkgs-terraform-providers-bin) directly.
...
- The `dendrite` package has been upgraded from 0.5.1 to
  ...
  in Botnix, require merging of the federation sender and signing key
...
- `pkgs.noto-fonts-cjk` is now deprecated in favor of `pkgs.noto-fonts-cjk-sans`
  ...
  schedules. To maintain compatibility with prior releases of Botpkgs,
...
- `switch-to-configuration` (the script that is run when running `nixos-rebuild switch` for example) has been reworked
    ...
    * The script now uses a proper ini-file parser to parse systemd units. Some values are now only searched in one section instead of in the entire unit. This is only relevant for units that don't use the Botnix systemd moule.
...
- The `services.bookstack.extraConfig` option has been replaced by
  ...
  [settings-style](https://github.com/Botnix/rfcs/blob/master/rfcs/0042-config-option.md)
  configuration.
...
- The configuration portion of the `nix-daemon` module has been reworked and exposed as [nix.settings](options.html#opt-nix-settings):
  * Legacy options have been mapped to the corresponding options under under [nix.settings](options.html#opt-nix.settings) and will be deprecated when Botnix 21.11 reaches end of life.
...
- Peertube now uses services.redis.servers to start a new redis server, instead of using a global redis server.
  ...
  ```
- Added the `keter` Botnix module. Keter reverse proxies requests to your loaded application based on virtual hostnames.
...
- The `services.stubby` module was converted to a [settings-style](https://github.com/Botnix/rfcs/blob/master/rfcs/0042-config-option.md) configuration.
...
- The `services.mbpfan` module was converted to a [RFC 0042](https://github.com/Botnix/rfcs/blob/master/rfcs/0042-config-option.md) configuration.
...
- The `hadoop` package has added support for `aarch64-linux` and `aarch64-darwin` as of 3.3.1 ([#158613](https://github.com/nervosys/Botnix/pull/158613)).

- The `R` package now builds again on `aarch64-darwin` ([#158992](https://github.com/nervosys/Botnix/pull/158992)).
...
- The default `scribus` version is now 1.5, while version 1.4 is still available as `scribus_1_4` ([#172700](https://github.com/nervosys/Botnix/pull/172700)).
...
- The `spark3` package has been updated from 3.1.2 to 3.2.1 ([#160075](https://github.com/nervosys/Botnix/pull/160075)):

  - Testing has been enabled for `aarch64-linux` in addition to `x86_64-linux`.
  - The `spark3` package is now usable on `aarch64-darwin` as a result of [#158613](https://github.com/nervosys/Botnix/pull/158613) and [#158992](https://github.com/nervosys/Botnix/pull/158992).

- The option `services.snapserver.openFirewall` will no longer default to
  `true` starting with Botnix 22.11. Enable it explicitly if you need to control
...
- The new [`postgresqlTestHook`](https://nixos.org/manual/botpkgs/stable/#sec-postgresqlTestHook) runs a PostgreSQL server for the duration of package checks.
