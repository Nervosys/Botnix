...
A Botnix test is a module that has the following structure:
...
in [`nodes.<name>`](#test-opt-nodes) are Botnix modules themselves.
...
[`nfs/simple.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/tests/nfs/simple.nix).
It uses two client nodes to test correct locking across server crashes.

## Calling a test {#sec-calling-nixos-tests}

Tests are invoked differently depending on whether the test is part of Botnix or lives in a different project.

### Testing within Botnix {#sec-call-nixos-test-in-nixos}

Tests that are part of Botnix are added to [`nixos/tests/all-tests.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/tests/all-tests.nix).
...
There are a few special Botnix options for test VMs:
...
:   The virtual networks to which the VM is connected. See
    [`nat.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/tests/nat.nix)
    for an example.
...
[`qemu-vm.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/modules/virtualisation/qemu-vm.nix).
