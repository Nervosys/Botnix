...
Botnix configuration option. It is invalid to define an option that
...
options = {
  name = mkOption {
    ...
    description = lib.mdDoc "Description for use in the Botnix manual.";
  };
};
```
...
:   An example value that will be shown in the Botnix manual.
...
-   Configuring the display managers independently by adding an enable
    option to every display manager module backend. (Botnix)
