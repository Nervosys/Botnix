# Writing Botnix Modules {#sec-writing-modules}

Botnix has a modular system for declarative configuration. This system
...
module
[`pam.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/modules/security/pam.nix)
declares the option `security.pam.services` that allows other modules (e.g.
[`sshd.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/modules/services/networking/ssh/sshd.nix))
to define PAM services; and it defines the option `environment.etc` (declared by
[`etc.nix`](https://github.com/nervosys/Botnix/blob/master/nixos/modules/system/etc/etc.nix))
...
options, but does not declare any. The structure of full Botnix modules
is shown in [Example: Structure of Botnix Modules](#ex-module-syntax).

::: {#ex-module-syntax .example}
### Structure of Botnix Modules
```nix
{ config, pkgs, ... }:
...
-   This `imports` list enumerates the paths to other Botnix modules that
...
[Example: Botnix Module for the "locate" Service](#locate-example)
...
### Botnix Module for the "locate" Service
```nix
{ config, lib, pkgs, ... }:
...
in {
  options.services.locate = {
    enable = mkOption {
      ...
      default = false;
      description = ''
        If enabled, Botnix will periodically update the database of
