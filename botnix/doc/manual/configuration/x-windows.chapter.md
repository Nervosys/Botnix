...
The X Window System (X11) provides the basis of Botnix' graphical user
...
Botnix's default *display manager* (the program that provides a graphical
...
Every display manager in Botnix supports auto-login, here is an example
...
Note: the use of `services.xserver.synaptics` is deprecated since Botnix
17.09.
...
prefer to keep the layout definitions inside the Botnix configuration.
:::
