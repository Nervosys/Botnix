...
in
{
  options = {
    ...
    system = {
      defaultChannel = mkOption {
        internal = true;
        type = types.str;
        default = "https://nixos.org/channels/botnix-unstable";
        description = lib.mdDoc "Default Botnix channel to which the root user is subscribed.";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = lib.mkIf cfg.channel.enable [
      ''f /root/.nix-channels - - - - ${config.system.defaultChannel} botnix\n''
    ];
  };
}
