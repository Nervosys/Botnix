...
See prior art:
- https://github.com/nervosys/Botnix/pull/194594
- https://github.com/nervosys/Botnix/pull/222236
- https://github.com/nervosys/Botnix/pull/222689
...
let
  ...
  makeCacheConf = { }:
    ...
    in
    pkgs.writeText "fc-00-botnix-cache.conf" ''
      <?xml version='1.0'?>
  ...
  # priority 10
  renderConf = pkgs.writeText "fc-10-botnix-rendering.conf" ''
    <?xml version='1.0'?>
    <!DOCTYPE fontconfig SYSTEM 'urn:fontconfig:fonts.dtd'>
    <fontconfig>
  ...
  defaultFontsConf =
    let genDefault = fonts: name:
      ...
      '';
    in
    pkgs.writeText "fc-52-botnix-default-fonts.conf" ''
    <?xml version='1.0'?>
    <!DOCTYPE fontconfig SYSTEM 'urn:fontconfig:fonts.dtd'>
    <fontconfig>
  ...
  # reject Type 1 fonts
  # priority 53
  rejectType1 = pkgs.writeText "fc-53-botnix-reject-type1.conf" ''
    <?xml version="1.0"?>
    <!DOCTYPE fontconfig SYSTEM "urn:fontconfig:fonts.dtd">
    <fontconfig>
  ...
  confPkg = pkgs.runCommand "fontconfig-conf" {
    ...
    # 00-botnix-cache.conf
    ln -s ${cacheConf}  $dst/00-botnix-cache.conf

    # 10-botnix-rendering.conf
    ln -s ${renderConf}       $dst/10-botnix-rendering.conf
    ...
    # 52-botnix-default-fonts.conf
    ln -s ${defaultFontsConf} $dst/52-botnix-default-fonts.conf

    # 53-no-bitmaps.conf
    ln -s ${rejectBitmaps} $dst/53-no-bitmaps.conf

    ${optionalString (!cfg.allowType1) ''
    # 53-botnix-reject-type1.conf
    ln -s ${rejectType1} $dst/53-botnix-reject-type1.conf
    ''}
  '';
