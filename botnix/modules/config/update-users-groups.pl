...
my $uidMapFile = "/var/lib/botnix/uid-map";
my $uidMap = -e $uidMapFile ? decode_json(read_file($uidMapFile)) : {};

my $gidMapFile = "/var/lib/botnix/gid-map";
...
make_path("/var/lib/botnix", { mode => 0755 }) unless $is_dry;
...
# current spec.
my $declGroupsFile = "/var/lib/botnix/declarative-groups";
my %declGroups;
$declGroups{$_} = 1 foreach split / /, -e $declGroupsFile ? read_file($declGroupsFile, { binmode => ":utf8" }) : "";

# Idem for the users.
my $declUsersFile = "/var/lib/botnix/declarative-users";
my %declUsers;
...
my $subUidMapFile = "/var/lib/botnix/auto-subuid-map";
