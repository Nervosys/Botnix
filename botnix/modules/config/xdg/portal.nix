    ...
    ({ config, lib, options, ... }:
      ...
      in
      {
        warnings = lib.mkIf config.xdg.portal.gtkUsePortal [
          "The option `${lib.showOption from}' defined in ${lib.showFiles fromOpt.files} has been deprecated. Setting the variable globally with `environment.sessionVariables' Botnix option can have unforeseen side-effects."
        ];
      }
    )
  ];
  ...
  options.xdg.portal = {
    ...
    xdgOpenUsePortal = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        See [#160923](https://github.com/nervosys/Botnix/issues/160923) for more info.
      '';
    };
  ...
  config =
    ...
    mkIf cfg.enable {
      ...
      environment = {
        ...
        # see https://github.com/nervosys/Botnix/issues/145174
