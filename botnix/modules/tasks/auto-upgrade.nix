...
in {
    ...
    system.autoUpgrade = {
      ...
      enable = mkOption {
        ...
        description = lib.mdDoc ''
          Whether to periodically upgrade Botnix to the latest
          version. If enabled, a systemd timer will run
          `botnix-rebuild switch --upgrade` once a
          day.
        '';
      };
      ...
      operation = mkOption {
        ...
        description = lib.mdDoc ''
          Whether to run
          `botnix-rebuild switch --upgrade` or run
          `botnix-rebuild boot --upgrade`
        '';
      };
      ...
      flake = mkOption {
        ...
        description = lib.mdDoc ''
          The Flake URI of the Botnix configuration to build.
      ...
      channel = mkOption {
        type = types.nullOr types.str;
        default = null;
        example = "https://nixos.org/channels/botnix-14.12-small";
        description = lib.mdDoc ''
          The URI of the Botnix channel to use for automatic
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.botnix-upgrade = {
      description = "Botnix Upgrade";
      ...
      script = let
        botnix-rebuild = "${config.system.build.botnix-rebuild}/bin/botnix-rebuild";
        date     = "${pkgs.coreutils}/bin/date";
        readlink = "${pkgs.coreutils}/bin/readlink";
        shutdown = "${config.systemd.package}/bin/shutdown";
        upgradeFlag = optional (cfg.channel == null) "--upgrade";
      in if cfg.allowReboot then ''
        ${botnix-rebuild} boot ${toString (cfg.flags ++ upgradeFlag)}
        ...
        if [ "''${booted}" = "''${built}" ]; then
          ${botnix-rebuild} ${cfg.operation} ${toString cfg.flags}
        ${optionalString (cfg.rebootWindow != null) ''
      ...
      '' else ''
        ${botnix-rebuild} ${cfg.operation} ${toString (cfg.flags ++ upgradeFlag)}
      '';
