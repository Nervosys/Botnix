    ...
    boot.zfs = {
      ...
      extraPools = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          Botnix's {option}`fileSystems` option, which makes Botnix automatically
          import the associated pool.

          However, in some cases (e.g. if you have many filesystems) it may be preferable
          to exclusively use ZFS commands to manage filesystems. If so, since Botnix/systemd
          will not be managing those filesystems, you will need to specify the ZFS pool here
          so that Botnix automatically imports it on every boot.
        '';
      };
      ...
      forceImportRoot = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          If you set this option to `false` and Botnix subsequently fails to
  ...
  config = mkMerge [
    (mkIf cfgZfs.enabled {
      ...
      boot = {
        ...
        # https://github.com/nervosys/Botnix/issues/106093
      ...
      # TODO FIXME See https://github.com/nervosys/Botnix/pull/99386#issuecomment-798813567. To not break people's bootloader and as probably not everybody would read release notes that thoroughly add inSystem.
      ...
      systemd.services = let
        ...
        createSyncService = pool:
          nameValuePair "zfs-sync-${pool}" {
            ...
            script = ''
              ${cfgZfs.package}/sbin/zfs set botnix:shutdown-time="$(date)" "${pool}"
            '';
          };
