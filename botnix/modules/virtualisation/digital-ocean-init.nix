...
in {
  ...
  options.virtualisation.digitalOcean.defaultConfigFile = mkOption {
    ...
    description = lib.mdDoc ''
      ...
      `/etc/botnix/configuration.nix` and be used when switching to
      a new configuration.
    '';
  };
  ...
  config = {
    systemd.services.digitalocean-init = mkIf cfg.rebuildFromUserData {
      ...
      unitConfig = {
        ConditionPathExists = "!/etc/botnix/do-userdata.nix";
      ...
      path = [ pkgs.jq pkgs.gnused pkgs.gnugrep config.systemd.package config.nix.package config.system.build.botnix-rebuild ];
      environment = {
        HOME = "/root";
        NIX_PATH = concatStringsSep ":" [
          "/nix/var/nix/profiles/per-user/root/channels/botnix"
      ...
      script = ''
        ...
        if jq -er '.user_data' /run/do-metadata/v1.json > $userData; then
          ...
          if nix-instantiate --parse $userData > /dev/null; then
            ...
            cp "$userData" /etc/botnix/do-userdata.nix
            if [[ ! -e /etc/botnix/configuration.nix ]]; then
              install -m0644 ${cfg.defaultConfigFile} /etc/botnix/configuration.nix
            fi
          else
