        ...
        example = lib.literalExpression ''
          {
            ...
            # create /etc/botnix/hostname.nix with a configuration for keeping the hostname applied
            "hostname-nix" = {
              enable = true;
              target = "/etc/botnix/hostname.nix";
            ...
            "configuration-nix" = {
              enable = true;
              target = "/etc/botnix/configuration.nix";
      ...
      contents = [
        {
          source = toYAML "metadata.yaml" {
            ...
            creation_date = 1;
            properties = {
              description = "${config.system.botnix.distroName} ${config.system.botnix.codeName} ${config.system.botnix.label} ${pkgs.system}";
              os = "${config.system.botnix.distroId}";
              release = "${config.system.botnix.codeName}";
            };
