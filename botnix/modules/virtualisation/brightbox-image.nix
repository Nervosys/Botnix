    ...
    pkgs.vmTools.runInLinuxVM (
      pkgs.runCommand "brightbox-image"
        { preVM =
          ...
          postVM =
            ''
              ...
              pushd $out
              ${pkgs.qemu_kvm}/bin/qemu-img convert -c -O qcow2 $diskImageBase botnix.qcow2
              rm $diskImageBase
              popd
            '';
          diskImageBase = "botnix-image-${config.system.botnix.label}-${pkgs.stdenv.hostPlatform.system}.raw";
        ...
        ''
          ...
          ${pkgs.e2fsprogs}/sbin/mkfs.ext4 -L botnix /dev/vda1
          ...
          # Create the system profile to allow botnix-rebuild to work.
          chroot /mnt ${config.nix.package.out}/bin/nix-env \
              -p /nix/var/nix/profiles/system --set ${config.system.build.toplevel} \
              --option build-users-group ""

          # `botnix-rebuild' requires an /etc/NIXOS.
          mkdir -p /mnt/etc
          touch /mnt/etc/NIXOS
          ...
          mkdir -p /mnt/etc/botnix /mnt/boot/grub
          cp ${./brightbox-config.nix} /mnt/etc/botnix/configuration.nix
