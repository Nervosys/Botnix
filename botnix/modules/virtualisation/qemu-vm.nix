# This module creates a virtual machine from the Botnix configuration.
# Building the `config.system.build.vm' attribute gives you a command
# that starts a KVM/QEMU VM running the Botnix configuration defined in
...
let
      ...
      ${lib.optionalString (cfg.useNixStoreImage)
        (if cfg.writableStore
          then ''
            ...
            ${qemu}/bin/qemu-img create -f qcow2 -F qcow2 -b ${storeImage}/botnix.qcow2 "$TMPDIR"/store.img
          ''
          else ''
            (
  ...
  # System image is akin to a complete Botnix install with
...

{
  ...
  options = {
    ...
    virtualisation.rootDevice =
      mkOption {
        ...
        example = "/dev/disk/by-label/botnix";
        description =
          lib.mdDoc ''
    ...
    virtualisation.forwardPorts = mkOption {
      ...
      description =
        lib.mdDoc ''
          ...
          ::: {.warning}
          If the Botnix firewall on the virtual machine is enabled, you also
    ...
    virtualisation.host.pkgs = mkOption {
      type = options.botpkgs.pkgs.type;
      default = pkgs;
      ...
      description = lib.mdDoc ''
        ...
        Changing this to e.g. a Darwin package set allows running Botnix VMs on Darwin.
      '';
    };
    ...
    virtualisation.useNixStoreImage =
      mkOption {
        ...
        description = lib.mdDoc ''
          ...
          with a full Botnix system image containing a Nix store and
  ...
  config = {
    ...
    # `botnix-rebuild build-vm`) of a system configuration, where the regular
    ...
    system.build.vm = hostPkgs.runCommand "botnix-vm" {
      ...
      ''
        ...
        ln -s ${hostPkgs.writeScript "run-botnix-vm" startVM} $out/bin/run-${config.system.name}-vm
      '';
