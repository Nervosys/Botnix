...

{
  ...
  options.security.sudo = {
    ...
    configFile = mkOption {
      type = types.lines;
      # Note: if syntax errors are detected in this file, the Botnix
  ...
  config = mkIf cfg.enable {
    assertions = [ {
      ...
      message = ''
        Botnix' `sudo` module does not support `sudo-rs`; see `security.sudo-rs` instead.
      '';
    } ];
    ...
    security.sudo.configFile = concatStringsSep "\n" (filter (s: s != "") [
      ''
        # Don't edit this file. Set the Botnix options ‘security.sudo.configFile’
