...
let
  ...
  in {
    ...
    renewService = lockfileName: {
      ...
      # https://github.com/nervosys/Botnix/pull/81371#issuecomment-605526099
      ...
      script = (if (lockfileName == null) then lib.id else wrapInFlock "${lockdir}${lockfileName}") ''
        ...
        if cmp -s domainhash.txt certificates/domainhash.txt && [ -e 'certificates/${keyName}.key' -a -e 'certificates/${keyName}.crt' -a -n "$(find accounts -name '${data.email}.key')" ]; then
          ...
          if ! lego ${renewOpts} --days ${toString data.validMinDays}; then
            if is_expiration_skippable out/full.pem; then
              echo 1>&2 "botnix-acme: Ignoring failed renewal because expiration isn't within the coming ${toString data.validMinDays} days"
            else
        ...
        # Otherwise do a full run
          ...
          # Produce a nice error for those doing their first botnix-rebuild with these certs
...
in {
    ...
    security.acme = {
      ...
      useRoot = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          owned certificates" in the Botnix manual, and only using this as a fallback
          or for testing.
        '';
      };
  ...
  imports = [
    ...
    '')
    (mkRemovedOptionModule [ "security" "acme" "directory" ] "ACME Directory is now hardcoded to /var/lib/acme and its permissions are managed by systemd. See https://github.com/nervosys/Botnix/issues/53852 for more info.")
