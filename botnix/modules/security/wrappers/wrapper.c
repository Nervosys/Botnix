...
static noreturn void assert_failure(const char *assertion) {
  fprintf(stderr, "Assertion `%s` in Botnix's wrapper.c failed.\n", assertion);
  fflush(stderr);
  abort();
}
...
static int make_caps_ambient(const char *self_path) {
  ...
  if (r < 0) {
    ...
    fprintf(stderr, "cannot get capabilities for %s: %s", self_path,
            strerror(errno));
    return 1;
  }
  ...
  switch (version) {
    ...
    default:
      fprintf(stderr,
              "BUG! Unsupported capability version 0x%x on %s. Report to "
              "Botnix bugtracker\n",
              version, self_path);
      return 1;
  }
  ...
  uint64_t set = user_data[0].permitted | (uint64_t)user_data[1].permitted
                                              << 32;
...
// Any future tunables are expected to only be accessible through
// GLIBC_TUNABLES.
//
...
int main(int argc, char **argv) {
  ...
  // argv[0] goes into a lot of places, to a far greater degree than other
  // elements of argv. glibc has had buffer overflows relating to argv[0], eg
  // CVE-2023-6246. Since we expect the wrappers to be invoked from either $PATH
  // or /run/wrappers/bin, there should be no reason to pass any particularly
  // large values here, so we can be strict for strictness' sake.
  ...
  //
  // glibc does this automatically in SUID binaries, but we'd like to cover
  // this:
  //
  ...
  for (char *unsec = UNSECURE_ENVVARS_TUNABLES UNSECURE_ENVVARS; *unsec;
