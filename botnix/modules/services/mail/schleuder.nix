      ...
      type = lib.types.submodule {
        ...
        options.keyserver = lib.mkOption {
          ...
          description = lib.mdDoc ''
            ...
            Note that Botnix uses a different default from upstream, since the upstream default sks-keyservers.net is deprecated.
          '';
    ...
    assertions = [
      {
        ...
        message = ''
          services.schleuder.settings.api.valid_api_keys is set. Defining API keys via Botnix config results in them being copied to the world-readable Nix store. Please use the extraSettingsFile option to store API keys in a non-public location.
        '';
      }
      {
        assertion = !(lib.any (db: db ? password) (lib.attrValues cfg.settings.database or {}));
        message = ''
          A password is defined for at least one database in services.schleuder.settings.database. Defining passwords via Botnix config results in them being copied to the world-readable Nix store. Please use the extraSettingsFile option to store database passwords in a non-public location.
        '';
      }
    ];
