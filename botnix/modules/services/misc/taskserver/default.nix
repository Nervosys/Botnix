...
let
  ...
  botnix-taskserver = with pkgs.python3.pkgs; buildPythonApplication {
    name = "botnix-taskserver";

    src = pkgs.runCommand "botnix-taskserver-src" { preferLocalBuild = true; } ''
      mkdir -p "$out"
...
in {
  options = {
    services.taskserver = {
      enable = mkOption {
        ...
        default = false;
        description = let
          url = "https://nixos.org/manual/botnix/stable/index.html#module-services-taskserver";
        in lib.mdDoc ''
          Whether to enable the Taskwarrior server.

          More instructions about Botnix in conjunction with Taskserver can be
          found [in the Botnix manual](${url}).
        '';
      };
  ...
  config = mkMerge [
    (mkIf cfg.enable {
      environment.systemPackages = [ botnix-taskserver ];
      ...
      systemd.services.taskserver = {
        ...
        preStart = let
          ...
          helperTool = "${botnix-taskserver}/bin/botnix-taskserver";
