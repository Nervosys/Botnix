...
let
  ...
  format = pkgs.formats.ini {
    # https://github.com/nervosys/Botnix/pull/121613#issuecomment-885241996
    listToValue = l:
  ...
  config = mkIf cfg.enable {
    warnings = []
      ++ (optional (head (cfg.settings.update_manager.enable_system_updates or [false])) ''
        Enabling system updates is not supported on Botnix and will lead to non-removable warnings in some clients.
      '')
    ...
    security.polkit.extraConfig = lib.optionalString cfg.allowSystemControl ''
      // botnix/moonraker: Allow Moonraker to perform system-level operations
      //
