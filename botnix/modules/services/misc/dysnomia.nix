...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    environment.variables = {
      DYSNOMIA_STATEDIR = "/var/state/dysnomia-botnix";
    ...
    dysnomia.package = pkgs.dysnomia.override (origArgs: dysnomiaFlags // lib.optionalAttrs (cfg.enableLegacyModules) {
      enableLegacy = builtins.trace ''
        ...
        disable legacy mode with the following Botnix configuration option:

        dysnomia.enableLegacyModules = false;

        In a future version of Dysnomia (and Botnix) the legacy option will go away!
      '' true;
    });
    ...
    dysnomia.properties = {
      ...
      supportedTypes = [
        ...
        # These are not base modules, but they are still enabled because they work with technology that are always enabled in Botnix
        "systemd-unit"
        "sysvinit-script"
        "botnix-configuration"
      ]
