    ...
    services.cloud-init = {
      enable = mkOption {
        ...
        description = mdDoc ''
          ...
          Botnix way of doing configuration, as configuration done by
          cloud-init might be overridden by a subsequent botnix-rebuild
          call. However, some parts of cloud-init fall outside of
          Botnix's responsibility, like filesystem resizing and ssh
          public key provisioning, and cloud-init is useful for that
          parts. Thus, be wary that using cloud-init in Botnix might
          come as some cost.
        '';
      };
