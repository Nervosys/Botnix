...
let
  ...
  jailConf =
    let
      configFile = configFormat.generate "jail.local" (
        { INCLUDES.before = "paths-botnix.conf"; } // (mapAttrs mkJailConfig attrsJails)
      );
  ...
  pathsConf = pkgs.writeText "paths-botnix.conf" ''
    # Botnix
...

{
    ...
    services.fail2ban = {
      ...
      jails = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          Botnix comes with a default `sshd` jail;
          for it to work well,
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc = {
      ...
      "fail2ban/paths-botnix.conf".source = pathsConf;
