  ...
  config = mkIf cfg.enable {
    ...
    networking.firewall = lib.mkIf cfg.openFirewall {
      extraCommands = ''
        ...
        ip46tables -A botnix-fw -p vrrp -m comment --comment "services.keepalived.openFirewall" -j ACCEPT
        ip46tables -A botnix-fw -p ah -m comment --comment "services.keepalived.openFirewall" -j ACCEPT
      '';

      extraStopCommands = ''
        ip46tables -D botnix-fw -p vrrp -m comment --comment "services.keepalived.openFirewall" -j ACCEPT
        ip46tables -D botnix-fw -p ah -m comment --comment "services.keepalived.openFirewall" -j ACCEPT
      '';
    };
