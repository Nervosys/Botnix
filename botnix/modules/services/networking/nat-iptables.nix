...
let
  ...
  flushNat = ''
    ${helpers}
    ip46tables -w -t nat -D PREROUTING -j botnix-nat-pre 2>/dev/null|| true
    ip46tables -w -t nat -F botnix-nat-pre 2>/dev/null || true
    ip46tables -w -t nat -X botnix-nat-pre 2>/dev/null || true
    ip46tables -w -t nat -D POSTROUTING -j botnix-nat-post 2>/dev/null || true
    ip46tables -w -t nat -F botnix-nat-post 2>/dev/null || true
    ip46tables -w -t nat -X botnix-nat-post 2>/dev/null || true
    ip46tables -w -t nat -D OUTPUT -j botnix-nat-out 2>/dev/null || true
    ip46tables -w -t nat -F botnix-nat-out 2>/dev/null || true
    ip46tables -w -t nat -X botnix-nat-out 2>/dev/null || true
  ...
  mkSetupNat = { iptables, dest, internalIPs, forwardPorts }: ''
    ...
    ${concatMapStrings (iface: ''
      ${iptables} -w -t nat -A botnix-nat-pre \
        -i '${iface}' -j MARK --set-mark 1
    '') cfg.internalInterfaces}

    # NAT the marked packets.
    ${optionalString (cfg.internalInterfaces != []) ''
      ${iptables} -w -t nat -A botnix-nat-post -m mark --mark 1 \
        ${optionalString (cfg.externalInterface != null) "-o ${cfg.externalInterface}"} ${dest}
    ''}

    # NAT packets coming from the internal IPs.
    ${concatMapStrings (range: ''
      ${iptables} -w -t nat -A botnix-nat-post \
    ...
    ${concatMapStrings (fwd: ''
      ...
      ${concatMapStrings (loopbackip:
        ...
        in ''
          ...
          ${iptables} -w -t nat -A botnix-nat-out \
  ...
  setupNat = ''
    ...
    ip46tables -w -t nat -N botnix-nat-pre
    ip46tables -w -t nat -N botnix-nat-post
    ip46tables -w -t nat -N botnix-nat-out
    ...
    ip46tables -w -t nat -A PREROUTING -j botnix-nat-pre
    ip46tables -w -t nat -A POSTROUTING -j botnix-nat-post
    ip46tables -w -t nat -A OUTPUT -j botnix-nat-out
  '';
