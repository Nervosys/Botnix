...
in
{
  ...
  imports = [
    ...
    (mkRemovedOptionModule [ "networking" "networkmanager" "firewallBackend" ] ''
      This option was removed as Botnix is now using iptables-nftables-compat even when using iptables, therefore Networkmanager now uses the nftables backend unconditionally.
    '')
  ];
  ...
  config = mkIf cfg.enable {
    ...
    # Turn off Botnix' network management when networking is managed entirely by NetworkManager
    networking = mkMerge [
