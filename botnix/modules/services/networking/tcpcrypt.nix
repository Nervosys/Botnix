  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.tcpcrypt = {
      ...
      preStart = ''
        ...
        iptables -t raw -N botnix-tcpcrypt
        iptables -t raw -A botnix-tcpcrypt -p tcp -m mark --mark 0x0/0x10 -j NFQUEUE --queue-num 666
        iptables -t raw -I PREROUTING -j botnix-tcpcrypt

        iptables -t mangle -N botnix-tcpcrypt
        iptables -t mangle -A botnix-tcpcrypt -p tcp -m mark --mark 0x0/0x10 -j NFQUEUE --queue-num 666
        iptables -t mangle -I POSTROUTING -j botnix-tcpcrypt
      '';
      ...
      postStop = ''
        ...
        iptables -t mangle -D POSTROUTING -j botnix-tcpcrypt || true
        iptables -t raw -D PREROUTING -j botnix-tcpcrypt || true

        iptables -t raw -F botnix-tcpcrypt || true
        iptables -t raw -X botnix-tcpcrypt || true

        iptables -t mangle -F botnix-tcpcrypt || true
        iptables -t mangle -X botnix-tcpcrypt || true
      '';
    };
  };
