...
let
  ...
  in pkgs.writeText "nextcloud-config.php" ''
      ...
      function nix_read_secret($file) {
        if (!file_exists($file)) {
          throw new \RuntimeException(sprintf(
            "Cannot start Nextcloud, secret file %s set by Botnix doesn't seem to "
    ...
    ${optionalString (cfg.secretFile != null) ''
      $CONFIG = array_replace_recursive($CONFIG, nix_decode_json_file(
        "${cfg.secretFile}",
        "Cannot start Nextcloud, secrets file %s set by Botnix doesn't exist!"
      ));
    ''}
  '';
in {
  ...
  config = mkIf cfg.enable (mkMerge [
    { warnings = let
        latest = 28;
        upgradeWarning = major: botnix:
          ''
            A legacy Nextcloud install (from before Botnix ${botnix}) may be installed.
