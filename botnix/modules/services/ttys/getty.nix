...

{
    ...
    services.getty = {
      ...
      greetingLine = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          The default shows current Botnix version label, machine type and tty.
        '';
      };
  ...
  config = {
    ...
    # botnix.label would not rebuild manual pages
    services.getty.greetingLine = mkDefault ''<<< Welcome to Botnix ${config.system.botnix.label} (\m) - \l >>>'';
    services.getty.helpLine = mkIf (config.documentation.botnix.enable && config.documentation.doc.enable) "\nRun 'botnix-help' for the Botnix manual.";
