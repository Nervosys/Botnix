...
let
  ...
  botnix-background-light = pkgs.botnix-artwork.wallpapers.nineish;
  botnix-background-dark = pkgs.botnix-artwork.wallpapers.nineish-dark-gray;

  botnix-gsettings-overrides = pkgs.budgie.budgie-gsettings-overrides.override {
    inherit (cfg) extraGSettingsOverrides extraGSettingsOverridePackages;
    inherit botnix-background-dark botnix-background-light;
  };

  botnix-background-info = pkgs.writeTextFile {
    ...
    text = ''
      ...
      <wallpapers>
        ...
        <wallpaper deleted="false">
          ...
          <filename>${botnix-background-dark.gnomeFilePath}</filename>
          <options>zoom</options>
          <shade_type>solid</shade_type>
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # GSettings overrides.
    environment.sessionVariables.NIX_GSETTINGS_OVERRIDES_DIR = "${botnix-gsettings-overrides}/share/gsettings-schemas/botnix-gsettings-overrides/glib-2.0/schemas";
