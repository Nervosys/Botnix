  ...
  mkExporterOpts = ({ name, port }: {
    ...
    firewallFilter = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        is true. It is used as `ip46tables -I botnix-fw firewallFilter -j botnix-fw-accept`.
      '';
    };
    user = mkOption {
      type = types.str;
  ...
  mkExporterConf = { name, conf, serviceOpts }:
    ...
    mkIf conf.enable {
      ...
      networking.firewall.extraCommands = mkIf conf.openFirewall (concatStrings [
        "ip46tables -A botnix-fw ${conf.firewallFilter} "
        "-m comment --comment ${name}-exporter -j botnix-fw-accept"
      ]);
