...
let
  ...
  pluginConfDir = pkgs.stdenv.mkDerivation {
    ...
    buildCommand = ''
      mkdir $out
      ln -s ${pluginConf} $out/botnix-config
    '';
  };
  ...
  # them afterwards in an attempt to get them to run on Botnix.
  ...
  # of munin-contrib in your botnix configuration.
    ...
    services.munin-node = {
      ...
      disabledPlugins = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          information, and the Botnix build of Munin does not write this file.
        '';
