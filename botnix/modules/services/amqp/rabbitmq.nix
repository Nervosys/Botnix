...
in
{
  ...
  imports = [
    (mkRemovedOptionModule [ "services" "rabbitmq" "cookie" ] ''
      ...
      Please remove it from your Botnix configuration and deploy a cookie securely instead.
      The renamed `unsafeCookie` must ONLY be used in isolated non-production environments such as Botnix VM tests.
    '')
  ];
    ...
    services.rabbitmq = {
      ...
      configItems = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          although outside Botnix that filename may have Erlang syntax, particularly
