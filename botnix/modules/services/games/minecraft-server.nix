...
let
  ...
  eulaFile = builtins.toFile "eula.txt" ''
    # eula.txt managed by Botnix Configuration
    eula=true
  '';
  ...
  serverPropertiesFile = pkgs.writeText "server.properties" (''
    # server.properties managed by Botnix configuration
  '' + concatStringsSep "\n" (mapAttrsToList
    ...
    systemd.services.minecraft-server = {
      ...
      '' + (if cfg.declarative then ''
        ...
        if [ -e .declarative ]; then
          ...
          echo "Autogenerated file that signifies that this server configuration is managed declaratively by Botnix" \
            > .declarative
