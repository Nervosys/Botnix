...
let
  ...
  configFile = format.generate "ASF.json" (cfg.settings // {
    ...
    # and botnix takes care of restarting the service
  ...
  options.services.archisteamfarm = {
    ...
    settings = lib.mkOption {
      ...
      description = lib.mdDoc ''
        ...
        Do note that `AutoRestart`  and `UpdateChannel` is always to `false` respectively `0` because Botnix takes care of updating everything.
