...
#
# See: https://github.com/Botnix/rfcs/pull/125
{ config
, pkgs
, lib
, ...
  ...
  schemas = {
    v1 = rec {
      ...
      json =
        ...
        (builtins.toJSON
          # Merge extensions first to not let them shadow Botnix bootspec data.
          (cfg.extensions //
          {
            "org.botnix.bootspec.v1" = {
              system = config.boot.kernelPackages.stdenv.hostPlatform.system;
              kernel = "${config.boot.kernelPackages.kernel}/${config.system.boot.loader.kernelFile}";
              kernelParams = config.boot.kernelParams;
              label = "${config.system.botnix.distroName} ${config.system.botnix.codeName} ${config.system.botnix.label} (Linux ${config.boot.kernelPackages.kernel.modDirVersion})";
      ...
      generator =
        let
          ...
          toplevelInjector = lib.escapeShellArgs [
            ...
            ''
              ."org.botnix.bootspec.v1".toplevel = $toplevel |
              ."org.botnix.bootspec.v1".init = $init
            ''
            "--sort-keys"
          ...
          specialisationInjector =
            ...
            lib.escapeShellArgs [
              "${pkgs.buildPackages.jq}/bin/jq"
              "--sort-keys"
              ''."org.botnix.specialisation.v1" = ($ARGS.named | map_values(. | first))''
