...

{
  ...
  options = {
    ...
    system.activationScripts = mkOption {
      ...
      description = lib.mdDoc ''
        A set of shell script fragments that are executed when a Botnix
        system configuration is activated.  Examples are updating
        /etc, creating accounts, and so on.  Since these are executed
        every time you boot the system or run
        {command}`botnix-rebuild`, it's important that they are
        idempotent and fast.
      '';
    ...
    system.userActivationScripts = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        service when a Botnix system configuration is activated. Examples are
    ...
    system.build.installBootLoader = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        See `botnix/modules/system/activation/switch-to-configuration.pl`.
      '';
      type = types.unique {
        message = ''
          Only one bootloader can be enabled at a time. This requirement has not
          been checked until Botnix 22.05. Earlier versions defaulted to the last
  ...
  config = {
    ...
    systemd.user = {
      services.botnix-activation = {
        description = "Run user-specific Botnix activation";
