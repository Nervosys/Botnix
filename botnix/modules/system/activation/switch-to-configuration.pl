...
my $start_list_file = "/run/botnix/start-list";
my $restart_list_file = "/run/botnix/restart-list";
my $reload_list_file = "/run/botnix/reload-list";
...
# This is the same as specifying a restart trigger in the Botnix module.
#
# The reload file asks the script to reload a unit. This is the same as
# specifying a reload trigger in the Botnix module and can be ignored if
# the unit is restarted in this activation.
my $restart_by_activation_file = "/run/botnix/activation-restart-list";
my $reload_by_activation_file = "/run/botnix/activation-reload-list";
my $dry_restart_by_activation_file = "/run/botnix/dry-activation-restart-list";
my $dry_reload_by_activation_file = "/run/botnix/dry-activation-reload-list";
...
# This is a Botnix installation if it has /etc/NIXOS or a proper
# /etc/os-release.
if (!-f "/etc/NIXOS" && (read_file("/etc/os-release", err_mode => "quiet") // "") !~ /^ID="?@distroId@"?/msx) {
    die("This is not a Botnix installation!\n");
}

make_path("/run/botnix", { mode => oct(755) });
open(my $stc_lock, '>>', '/run/botnix/switch-to-configuration.lock') or die "Could not open lock - $!";
flock($stc_lock, LOCK_EX) or die "Could not acquire lock - $!";
openlog("botnix", "", LOG_USER);
...
if ($new_init_interface_version ne $cur_init_interface_version) {
    print STDERR <<'EOF';
Warning: the new Botnix configuration has an ‘init’ that is
...
sub handle_modified_unit { ## no critic(Subroutines::ProhibitManyArgs, Subroutines::ProhibitExcessComplexity)
    ...
    if ($unit eq "sysinit.target" || $unit eq "basic.target" || $unit eq "multi-user.target" || $unit eq "graphical.target" || $unit =~ /\.path$/msx || $unit =~ /\.slice$/msx) {
        ...
        # FIXME: do something?
        # Attempt to fix this: https://github.com/nervosys/Botnix/pull/141192
        # Revert of the attempt: https://github.com/nervosys/Botnix/pull/147609
        # More details: https://github.com/nervosys/Botnix/issues/74899#issuecomment-981142430
    } else {
...
while (my $f = <$list_active_users>) {
    ...
    system("@su@", "-s", "@shell@", "-l", $name, "-c",
           ...
           "$new_systemd/bin/systemctl --user start botnix-activation.service");
}
