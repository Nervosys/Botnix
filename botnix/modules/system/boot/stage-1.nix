...
let
  ...
  udevRules = pkgs.runCommand "udev-rules" {
      ...
      # in the Botnix installation CD, so use ID_CDROM_MEDIA in the
  ...
  initialRamdisk = pkgs.makeInitrd {
    ...
    contents =
      [ { object = bootStage1;
        ...
        { object = config.environment.etc."modprobe.d/botnix.conf".source;
          symlink = "/etc/modprobe.d/botnix.conf";
        }
...

{
  options = {
    ...
    boot.initrd.enable = mkOption {
      ...
      description = lib.mdDoc ''
        Whether to enable the Botnix initial RAM disk (initrd). This may be
    ...
    boot.initrd.compressor = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        - A function which, given the botpkgs package set, returns the path to a compressor tool, e.g. `pkgs: "''${pkgs.pigz}/bin/pigz"`
    ...
    boot.initrd.verbose = mkOption {
      ...
      description =
        lib.mdDoc ''
          ...
          only the mandatory messages generated by the Botnix scripts. For a
  ...
  config = mkIf config.boot.initrd.enable {
    ...
    system.build = mkMerge [
      ...
      # generated in botnix/modules/system/boot/systemd/initrd.nix
