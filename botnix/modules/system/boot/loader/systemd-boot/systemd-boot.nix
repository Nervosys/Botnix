  ...
  systemdBootBuilder = pkgs.substituteAll {
    ...
    copyExtraFiles = pkgs.writeShellScript "copy-extra-files" ''
      ...
      ${concatStrings (mapAttrsToList (n: v: ''
        ...
        ${pkgs.coreutils}/bin/install -D $empty_file "${efi.efiSysMountPoint}/efi/botnix/.extra-files/"${escapeShellArg n}
      '') cfg.extraFiles)}

      ${concatStrings (mapAttrsToList (n: v: ''
        ${pkgs.coreutils}/bin/install -Dp "${pkgs.writeText n v}" "${efi.efiSysMountPoint}/loader/entries/"${escapeShellArg n}
        ${pkgs.coreutils}/bin/install -D $empty_file "${efi.efiSysMountPoint}/efi/botnix/.extra-files/loader/entries/"${escapeShellArg n}
      '') cfg.extraEntries)}
    '';
  };
    ...
    memtest86 = {
      ...
      entryFilename = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          so if you want something to appear after all the Botnix entries,
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      ++ concatMap (filename: [
        ...
        {
          assertion = !(hasInfix "botnix/.extra-files" (toLower filename));
          message = "boot.loader.systemd-boot.extraFiles.${lib.strings.escapeNixIdentifier filename} is invalid: files cannot be placed in the botnix/.extra-files directory";
        }
