...
def bootspec_from_json(bootspec_json: Dict) -> BootSpec:
    specialisations = bootspec_json['org.botnix.specialisation.v1']
    specialisations = {k: bootspec_from_json(v) for k, v in specialisations.items()}
    return BootSpec(**bootspec_json['org.botnix.bootspec.v1'], specialisations=specialisations)
...
def copy_from_file(file: str, dry_run: bool = False) -> str:
    ...
    efi_file_path = "/efi/botnix/%s-%s.efi" % (store_dir, suffix)
    if not dry_run:
...
def remove_old_entries(gens: list[SystemIdentifier]) -> None:
    rex_profile = re.compile(r"^" + re.escape(EFI_SYS_MOUNT_POINT) + "/loader/entries/botnix-(.*)-generation-.*\.conf$")
    rex_generation = re.compile(r"^" + re.escape(EFI_SYS_MOUNT_POINT) + "/loader/entries/botnix.*-generation-([0-9]+)(-specialisation-.*)?\.conf$")
    known_paths = []
    for gen in gens:
        bootspec = get_bootspec(gen.profile, gen.generation)
        known_paths.append(copy_from_file(bootspec.kernel, True))
        known_paths.append(copy_from_file(bootspec.initrd, True))
    for path in glob.iglob(f"{EFI_SYS_MOUNT_POINT}/loader/entries/botnix*-generation-[1-9]*.conf"):
        ...
        if not (prof, gen_number, None) in gens:
            os.unlink(path)
    for path in glob.iglob(f"{EFI_SYS_MOUNT_POINT}/efi/botnix/*"):
...
def install_bootloader(args: argparse.Namespace) -> None:
    ...
    os.makedirs(f"{EFI_SYS_MOUNT_POINT}/efi/botnix", exist_ok=True)
    ...
    for gen in gens:
        try:
            ...
            # See https://github.com/nervosys/Botnix/issues/114552
    ...
    for root, _, files in os.walk(f"{EFI_SYS_MOUNT_POINT}/efi/botnix/.extra-files", topdown=False):
        relative_root = root.removeprefix(f"{EFI_SYS_MOUNT_POINT}/efi/botnix/.extra-files").removeprefix("/")
    ...
    os.makedirs(f"{EFI_SYS_MOUNT_POINT}/efi/botnix/.extra-files", exist_ok=True)
