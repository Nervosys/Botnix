...
let
  ...
  grubConfig = args:
    ...
    { splashImage = f cfg.splashImage;
      ...
      bootloaderId = if args.efiBootloaderId == null then "${config.system.botnix.distroName}${efiSysMountPoint'}" else args.efiBootloaderId;
  ...
  defaultSplash = pkgs.botnix-artwork.wallpapers.simple-dark-gray-bootloader.gnomeFilePath;
in
  ...
  options = {
    ...
    boot.loader.grub = {
          ...
          options = {
            ...
            efiBootloaderId = mkOption {
              ...
              description = lib.mdDoc ''
                ...
                The default is to name it Botnix and append the path or efiSysMountPoint.
      ...
      extraPerEntryConfig = mkOption {
        ...
        description = lib.mdDoc ''
          Additional GRUB commands inserted in the configuration file
          at the start of each Botnix menu entry.
        '';
      };
      ...
      entryOptions = mkOption {
        default = "--class botnix --unrestricted";
        type = types.nullOr types.str;
        description = lib.mdDoc ''
          Options applied to the primary Botnix menu entry.
        '';
      };

      subEntryOptions = mkOption {
        default = "--class botnix";
        type = types.nullOr types.str;
        description = lib.mdDoc ''
          Options applied to the secondary Botnix submenu entry.
        '';
      };

      theme = mkOption {
        type = types.nullOr types.path;
        example = literalExpression "pkgs.botnix-grub2-theme";
        default = null;
        description = lib.mdDoc ''
      ...
      efiInstallAsRemovable = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          *not* booted in UEFI mode at the time GRUB is being installed, the
          ...
          - You are installing Botnix and want it to boot in UEFI mode,
      ...
      forcei686 = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          to install and run Botnix on 64bit x86 systems with 32bit (U)EFI.
        '';
      };
      ...
      system.build.installBootLoader =
        let
          install-grub-pl = pkgs.substituteAll {
            ...
            inherit (config.system.botnix) distroName;
          };
