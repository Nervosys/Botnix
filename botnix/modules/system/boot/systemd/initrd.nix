  ...
  config = mkIf (config.boot.initrd.enable && cfg.enable) {
    ...
    boot.initrd.systemd = {
      ...
      contents = {
        ...
        "/etc/modules-load.d/botnix.conf".text = concatStringsSep "\n" config.boot.initrd.kernelModules;
        ...
        "/etc/sysctl.d/botnix.conf".text = "kernel.modprobe = /sbin/modprobe";
      ...
      } // optionalAttrs (config.environment.etc ? "modprobe.d/botnix.conf") {
        "/etc/modprobe.d/botnix.conf".source = config.environment.etc."modprobe.d/botnix.conf".source;
      };
      ...
      services.initrd-botnix-activation = {
        after = [ "initrd-fs.target" ];
        requiredBy = [ "initrd.target" ];
        unitConfig.AssertPathExists = "/etc/initrd-release";
        serviceConfig.Type = "oneshot";
        description = "Botnix Activation";
        ...
        script = /* bash */ ''
          ...
          # If we are not booting a Botnix closure (e.g. init=/bin/sh),
          ...
          if ! [ -x "/sysroot$(readlink "/sysroot$closure/prepare-root" || echo "$closure/prepare-root")" ]; then
            ...
            echo "$closure does not look like a Botnix installation - not activating"
            exit 0
          fi
      ...
      # is the case when not booting a Botnix system) or with an empty string, causing
