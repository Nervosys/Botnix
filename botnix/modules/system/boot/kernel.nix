...
let
  ...
  kernelModulesConf = pkgs.writeText "botnix.conf"
    ''
  ...
  options = {
    ...
    boot.kernel.features = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        which would have separate botnix options.
    ...
    boot.kernelPackages = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        [in the Linux section of the manual](https://nixos.org/manual/botnix/unstable/index.html#sec-kernel-config).
      '';
    };
      ...
      description = lib.mdDoc ''
        ...
        ```nix
        {
          ...
          features = {                  # attrset of extra "features" the kernel is considered to have
            foo = true;                 # (may be checked by other Botnix modules, optional)
          };
    ...
    boot.kernel.randstructSeed = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        only enabled in Botnix hardened kernels. Using a custom seed requires
    ...
    [ (mkIf config.boot.initrd.enable {
        boot.initrd.availableKernelModules =
          optionals config.boot.initrd.includeDefaultModules ([
            ...
            # shouldn't be included by default since botnix-generate-config
      ...
      (mkIf config.boot.kernel.enable {
        ...
        # Create /etc/modules-load.d/botnix.conf, which is read by
        # systemd-modules-load.service to load required kernel modules.
        environment.etc =
          { "modules-load.d/botnix.conf".source = kernelModulesConf;
          };
        ...
        # botpkgs kernels are assumed to have all required features
