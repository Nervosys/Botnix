...
# in the current one.  For efficiency, don't look under /etc/botnix
# (where all the Botnix sources live).
sub cleanup {
    if ($File::Find::name eq "/etc/botnix") {
        $File::Find::prune = 1;
        return;
    }
    if (-l $_) {
...
sub link {
    ...
    # botnix-enter sets up /etc/resolv.conf as a bind mount, so skip it.
