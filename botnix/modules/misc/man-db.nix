...

{
    ...
    documentation.man.man-db = {
      ...
      skipPackages = lib.mkOption {
        ...
        description = lib.mdDoc ''
          ...
          This can be useful to avoid unnecessary rebuilds due to packages that change frequently, like botnix-version.
        '';
      };
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."man_db.conf".text =
      ...
      in
      ''
        # Manual pages paths for Botnix
        MANPATH_MAP /run/current-system/sw/bin /run/current-system/sw/share/man
        MANPATH_MAP /run/wrappers/bin          /run/current-system/sw/share/man

        ${lib.optionalString config.documentation.man.generateCaches ''
        # Generated manual pages cache for Botnix (immutable)
        MANDB_MAP /run/current-system/sw/share/man ${manualCache}
        ''}
        # Manual pages caches for Botnix
        MANDB_MAP /run/current-system/sw/share/man /var/cache/man/botnix
      '';
  };
}
