...
for (my $n = 0; $n < scalar @ARGV; $n++) {
    my $arg = $ARGV[$n];
    if ($arg eq "--help") {
        exec "man botnix-generate-config" or die;
    }
    elsif ($arg eq "--dir") {
        $n++;
        $outDir = $ARGV[$n];
...
# Pull in Botnix configuration for containers.
...
EOF
            ...
            $bootLoaderConfig = <<EOF;
  # Use the extlinux boot loader. (Botnix wants to enable GRUB by default)
...
@configuration@
EOF
        print STDERR "For more hardware-specific settings, see https://github.com/Botnix/botnix-hardware.\n"
    } else {
