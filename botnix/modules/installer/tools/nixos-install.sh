...
# Get the path of the Botnix configuration file.
if [[ -z $BOTNIX_CONFIG ]]; then
    BOTNIX_CONFIG=$mountPoint/etc/botnix/configuration.nix
fi

if [[ ${BOTNIX_CONFIG:0:1} != / ]]; then
    echo "$0: \$BOTNIX_CONFIG is not an absolute path"
    exit 1
fi
...
if [[ -n $flake ]]; then
    ...
    if [[ -z "$flakeAttr" ]]; then
        echo "Please specify the name of the Botnix configuration to be installed, as a URI fragment in the flake-uri."
...
if [[ ! -e $BOTNIX_CONFIG && -z $system && -z $flake ]]; then
    echo "configuration file $BOTNIX_CONFIG doesn't exist"
    exit 1
fi
...
if [[ -z $noChannelCopy ]]; then
    if [[ -z $channelPath ]]; then
        channelPath="$(nix-env -p /nix/var/nix/profiles/per-user/root/channels -q botnix --no-name --out-path 2>/dev/null || echo -n "")"
    fi
...
# Mark the target as a Botnix installation, otherwise switch-to-configuration will chicken out.
...
if [[ -z $noBootLoader ]]; then
    ...
    export mountPoint
    NIXOS_INSTALL_BOOTLOADER=1 botnix-enter --root "$mountPoint" -c "$(cat <<'EOF'
...
if [[ -z $noRootPasswd ]] && [ -t 0 ]; then
    if botnix-enter --root "$mountPoint" -c 'test -e /nix/var/nix/profiles/system/sw/bin/passwd'; then
        set +e
        botnix-enter --root "$mountPoint" -c 'echo "setting root password..." && /nix/var/nix/profiles/system/sw/bin/passwd'
        exit_code=$?
        set -e

        if [[ $exit_code != 0 ]]; then
            echo "Setting a root password failed with the above printed error."
            echo "You can set the root password manually by executing \`botnix-enter --root ${mountPoint@Q}\` and then running \`passwd\` in the shell of the new system."
            exit $exit_code
        fi
    fi
fi
