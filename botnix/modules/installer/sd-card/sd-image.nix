# This module creates a bootable SD card image containing the given Botnix
...
in
{
  ...
  options.sdImage = {
    imageName = mkOption {
      default = "${config.sdImage.imageBaseName}-${config.system.botnix.label}-${pkgs.stdenv.hostPlatform.system}.img";
    ...
    rootPartitionUUID = mkOption {
      ...
      description = lib.mdDoc ''
        UUID for the filesystem on the main Botnix partition on the SD card.
      '';
    };
  ...
  config = {
    ...
    boot.postBootCommands = lib.mkIf config.sdImage.expandOnBoot ''
      ...
      if [ -f /nix-path-registration ]; then
        ...
        # botnix-rebuild also requires a "system" profile and an /etc/NIXOS tag.
        touch /etc/NIXOS
