# This module creates a bootable ISO image containing the given Botnix
...
let
  ...
  let
    finalCfg = {
      name = "${config.isoImage.prependToMenuLabel}${config.system.botnix.distroName} ${config.system.botnix.label}${config.isoImage.appendToMenuLabel}";
  ...
  baseIsolinuxCfg = ''
    ...
    LABEL boot
    MENU LABEL ${config.isoImage.prependToMenuLabel}${config.system.botnix.distroName} ${config.system.botnix.label}${config.isoImage.appendToMenuLabel}
    ...
    MENU LABEL ${config.isoImage.prependToMenuLabel}${config.system.botnix.distroName} ${config.system.botnix.label}${config.isoImage.appendToMenuLabel} (nomodeset)
    LINUX /boot/${config.system.boot.loader.kernelFile}
    ...
    MENU LABEL ${config.isoImage.prependToMenuLabel}${config.system.botnix.distroName} ${config.system.botnix.label}${config.isoImage.appendToMenuLabel} (copytoram)
    LINUX /boot/${config.system.boot.loader.kernelFile}
    ...
    LABEL boot-debug
    MENU LABEL ${config.isoImage.prependToMenuLabel}${config.system.botnix.distroName} ${config.system.botnix.label}${config.isoImage.appendToMenuLabel} (debug)
    LINUX /boot/${config.system.boot.loader.kernelFile}
    ...
    LABEL boot-serial
    MENU LABEL ${config.isoImage.prependToMenuLabel}${config.system.botnix.distroName} ${config.system.botnix.label}${config.isoImage.appendToMenuLabel} (serial console=ttyS0,115200n8)
    LINUX /boot/${config.system.boot.loader.kernelFile}
  ...
  grubMenuCfg = ''
    ...
    search --set=root --file /EFI/botnix-installer-image
  ...
  efiDir = pkgs.runCommand "efi-directory" {
    ...
    touch $out/EFI/botnix-installer-image
    ...
    # line. We need this to allow the botnix iso to be booted from grub directly.
    if [ \''${iso_path} ] ; then
...

{
  options = {
    ...
    isoImage.isoBaseName = mkOption {
      default = config.system.botnix.distroId;
      type = lib.types.str;
      description = lib.mdDoc ''
    ...
    isoImage.volumeID = mkOption {
      # botnix-$EDITION-$RELEASE-$ARCH
      default = "botnix${optionalString (config.isoImage.edition != "") "-${config.isoImage.edition}"}-${config.system.botnix.release}-${pkgs.stdenv.hostPlatform.uname.processor}";
      type = lib.types.str;
      description = lib.mdDoc ''
    ...
    isoImage.efiSplashImage = mkOption {
      default = pkgs.fetchurl {
          url = "https://raw.githubusercontent.com/Botnix/botnix-artwork/a9e05d7deb38a8e005a2b52575a3f59a63a4dba0/bootloader/efi-background.png";
    ...
    isoImage.splashImage = mkOption {
      default = pkgs.fetchurl {
          url = "https://raw.githubusercontent.com/Botnix/botnix-artwork/a9e05d7deb38a8e005a2b52575a3f59a63a4dba0/bootloader/isolinux/bios-boot.png";
    ...
    isoImage.grubTheme = mkOption {
      default = pkgs.botnix-grub2-theme;
    ...
    isoImage.syslinuxTheme = mkOption {
      default = ''
        MENU TITLE ${config.system.botnix.distroName}
    ...
    isoImage.prependToMenuLabel = mkOption {
      ...
      example = "Install ";
      description = lib.mdDoc ''
        The string to prepend before the menu label for the Botnix system.
        This will be directly prepended (without whitespace) to the Botnix version
    ...
    isoImage.appendToMenuLabel = mkOption {
      ...
      example = " Live System";
      description = lib.mdDoc ''
        The string to append after the menu label for the Botnix system.
        This will be directly appended (without whitespace) to the Botnix version
        string, like for example if it is set to `XXX`:

        `Botnix 99.99-pre666XXX`
      '';
    };
  ...
  config = {
    ...
    isoImage.contents =
      [
        ...
        { source = pkgs.writeText "version" config.system.botnix.label;
          target = "/version.txt";
        }
    ...
    boot.postBootCommands =
      ''
        ...
        # botnix-rebuild also requires a "system" profile and an
        # /etc/NIXOS tag.
        touch /etc/NIXOS
