...
- run `botnix-install` in a temporary folder
- transfer from the temporary store the additional paths registered to the installed Botnix
...
, # The Botnix configuration to be installed onto the disk image.
  config
...
, # Filesystem label
  label ? if onlyNixStore then "nix-store" else "botnix"

, # The initial Botnix configuration file to be copied to
...
let format' = format; in let
  ...
  binPath = with pkgs; makeBinPath (
    [ rsync
      ...
      e2fsprogs
      lkl
      config.system.build.botnix-install
      config.system.build.botnix-enter
      nix
      systemdMinimal
    ]
  ...
  prepareImage = ''
    ...
    # https://github.com/nervosys/Botnix/issues/23052.
    set -f
    ...
    # Provide a Nix database so that botnix-install can copy closures.
    export NIX_STATE_DIR=$TMPDIR/state
    nix-store --load-db < ${closureInfo}/registration

    chmod 755 "$TMPDIR"
    echo "running botnix-install..."
    botnix-install --root $root --no-bootloader --no-root-passwd \
  ...
  buildImage = pkgs.vmTools.runInLinuxVM (
    pkgs.runCommand name {
      ...
      mkdir -p /mnt/etc/botnix
      ${optionalString (configFile != null) ''
        cp ${configFile} /mnt/etc/botnix/configuration.nix
      ''}
      ...
      ${lib.optionalString installBootLoader ''
        ...
        NIXOS_INSTALL_BOOTLOADER=1 botnix-enter --root $mountPoint -- /nix/var/nix/profiles/system/bin/switch-to-configuration boot
      ...
      for ((i = 0; i < ''${#targets_[@]}; i++)); do
        ...
        if [ -n "$user$group" ]; then
          # We have to botnix-enter since we need to use the user and group of the VM
          botnix-enter --root $mountPoint -- chown -R "$user:$group" "$target"
        fi
      done
