...
# fix up declaration paths in lazy options, since we don't eval them from a full botpkgs dir
for (k, v) in options.items():
    # The _module options are not declared in botnix/modules
    if v.value['loc'][0] != "_module":
        v.value['declarations'] = list(map(lambda s: f'botnix/modules/{s}' if isinstance(s, str) else s, v.value['declarations']))
...
for (k, v) in options.items():
    ...
    if v.value.get('type', "unspecified") == "unspecified":
        ...
        print(
            ...
            "https://nixos.org/manual/botnix/stable/index.html#sec-option-types\x1b[0m", file=sys.stderr)
...
if hasWarnings and warningsAreErrors:
    print(
        "\x1b[1;31m" +
        "Treating warnings as errors. Set documentation.botnix.options.warningsAreErrors " +
