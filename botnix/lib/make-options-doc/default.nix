/* Generate JSON, XML and DocBook documentation for given Botnix options.
   ...
   Minimal example:
    ...
    let
      eval = import (pkgs.path + "/botnix/lib/eval-config.nix") {
        baseModules = [
          ../module.nix
        ];
        modules = [];
...
# used to split the options doc build into a static part (botnix/modules) and a dynamic part
# (non-botnix modules imported via configuration.nix, other module sources).
...
in rec {
  ...
  optionsAsciiDoc = pkgs.runCommand "options.adoc" {
    nativeBuildInputs = [ pkgs.botnix-render-docs ];
  } ''
    botnix-render-docs -j $NIX_BUILD_CORES options asciidoc \
      --manpage-urls ${pkgs.path + "/doc/manpage-urls.json"} \
      --revision ${lib.escapeShellArg revision} \
      ${optionsJSON}/share/doc/botnix/options.json \
      $out
  '';
  ...
  optionsCommonMark = pkgs.runCommand "options.md" {
    ...
    botnix-render-docs -j $NIX_BUILD_CORES options commonmark \
      --manpage-urls ${pkgs.path + "/doc/manpage-urls.json"} \
      --revision ${lib.escapeShellArg revision} \
      ${optionsJSON}/share/doc/botnix/options.json \
      $out
  ...
  optionsJSON = pkgs.runCommand "options.json"
    { meta.description = "List of Botnix options in JSON format";
      nativeBuildInputs = [
        pkgs.brotli
        pkgs.python3
      ];
  ...
  optionsDocBook = lib.warn "optionsDocBook is deprecated since 23.11 and will be removed in 24.05"
    (pkgs.runCommand "options-docbook.xml" {
      nativeBuildInputs = [
        pkgs.botnix-render-docs
      ];
    } ''
      botnix-render-docs -j $NIX_BUILD_CORES options docbook \
        --manpage-urls ${pkgs.path + "/doc/manpage-urls.json"} \
        --revision ${lib.escapeShellArg revision} \
