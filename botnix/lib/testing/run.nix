...
in
{
  options = {
    ...
    test = mkOption {
      ...
      description = mdDoc ''
        ...
        This implies that Botnix tests run isolated from the network, making them
        more dependable.
      '';
    };
  };
  ...
  config = {
    rawTestDerivation = hostPkgs.stdenv.mkDerivation {
      ...
      requiredSystemFeatures = [ "kvm" "botnix-test" ];
      ...
      buildCommand = ''
        ...
        ${config.driver}/bin/botnix-test-driver -o $out
      '';
