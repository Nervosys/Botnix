...
let
  ...
  driver =
    hostPkgs.runCommand "botnix-test-driver-${config.name}"
      {
        ...
        meta = config.meta // {
          mainProgram = "botnix-test-driver";
        };
      }
      ''
        mkdir -p $out/bin
        ...
        ${lib.optionalString (!config.skipTypeCheck) ''
          ...
          echo "See https://nixos.org/manual/botnix/stable/#test-opt-skipTypeCheck"
        ...
        ln -s ${testDriver}/bin/botnix-test-driver $out/bin/botnix-test-driver

        ${testDriver}/bin/generate-driver-symbols
        ${lib.optionalString (!config.skipLint) ''
          echo "Linting test script (enable/disable: config.skipLint)"
          echo "See https://nixos.org/manual/botnix/stable/#test-opt-skipLint"
...
in
{
  options = {
    ...
    skipTypeCheck = mkOption {
      ...
      default = false;
      description = mdDoc ''
        Disable type checking. This must not be enabled for new Botnix tests.
