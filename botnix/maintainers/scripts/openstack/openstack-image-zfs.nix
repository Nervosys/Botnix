...
in
{
  ...
  options.openstackImage = {
    name = mkOption {
      ...
      default = "botnix-openstack-image-${config.system.botnix.label}-${pkgs.stdenv.hostPlatform.system}";
    };
  ...
  config = {
    ...
    system.build.openstackImage = import ../../../lib/make-single-disk-zfs-image.nix {
      ...
      postVM = ''
        ...
        ${pkgs.jq}/bin/jq -n \
          --arg system_label ${lib.escapeShellArg config.system.botnix.label} \
