...
# Uploads and registers Botnix images built from the
# <botnix/release.nix> amazonImage attribute. Images are uploaded and
...
# result of the amazon-image from botnix/release.nix
store_path=$1
...
image_name="Botnix-${image_label}-${image_system}"
image_description="Botnix ${image_label} ${image_system}"
    ...
    for image_file in "${image_files[@]}"; do
        ...
        if [ -z "$task_id" ]; then
            ...
            task_id=$(aws ec2 import-snapshot --role-name "$service_role_name" --disk-container "{
              \"Description\": \"botnix-image-${image_label}-${image_system}\",
              \"Format\": \"vhd\",
              \"UserBucket\": {
                  \"S3Bucket\": \"$bucket\",
                  \"S3Key\": \"$aws_path\"
