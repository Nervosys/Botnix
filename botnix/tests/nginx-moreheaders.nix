import ./make-test-python.nix {
    ...
    webserver = { pkgs, ... }: {
      services.nginx = {
        ...
        virtualHosts.test = {
          ...
          extraConfig = ''
            more_set_headers "X-Powered-By: botnix";
          '';
        };
      };
    };
  ...
  testScript = ''
    ...
    webserver.succeed("curl --fail --resolve test:80:127.0.0.1 --head --verbose http://test | grep -q \"X-Powered-By: botnix\"")
    webserver.fail("curl --fail --resolve test:80:127.0.0.1 --head --verbose http://test | grep -q \"Referrer-Policy: no-referrer\"")

    webserver.succeed("curl --fail --resolve test:80:127.0.0.1 --head --verbose http://test/some | grep -q \"X-Powered-By: botnix\"")
