    ...
    botnix.unstable.x86_64 =
      let
        systemConfig = { pkgs, ... }: {
          ...
          documentation.botnix.enable = false;
        ...
        qemuConfig = { pkgs, ... }: {
          ...
          fileSystems."/".device = "/dev/disk/by-label/botnix";
        ...
        config = (import (pkgs.path + "/botnix/lib/eval-config.nix") {
          inherit pkgs; modules = [ qemuConfig ];
          system = "x86_64-linux";
        }).config;
      in
      import (pkgs.path + "/botnix/lib/make-disk-image.nix") {
