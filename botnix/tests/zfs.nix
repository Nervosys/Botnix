  ...
  makeZfsTest = name:
    ...
    makeTest {
      ...
      nodes.machine = { config, pkgs, lib, ... }:
        ...
        # /dev/disk/by-id doesn't get populated in the Botnix test framework
      ...
      testScript = ''
        ...
        with subtest("sharesmb works"):
            machine.succeed(
                ...
                "bootctl set-default botnix-generation-1-specialisation-samba.conf",
                "sync",
            )
            machine.crash()
            machine.wait_for_unit("multi-user.target")
        ...
        with subtest("encryption works"):
            machine.succeed(
                ...
                "bootctl set-default botnix-generation-1-specialisation-encryption.conf",
                "sync",
        ...
        with subtest("boot.zfs.forceImportAll works"):
            machine.succeed(
                ...
                "bootctl set-default botnix-generation-1-specialisation-forcepool.conf",
                "rm /etc/hostid",
                "sync",
            )
            machine.crash()
