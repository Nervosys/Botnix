/*
  ...
  - For this Botnix test, we *had* to store some DB-related and
...
import ./make-test-python.nix ({ pkgs, ... }:
  ...
  send-toot = pkgs.writeScriptBin "send-toot" ''
    ...
    echo "jamy-password" | toot login_cli -i "pleroma.botnix.test" -e "jamy@botnix.test"
    echo "Login OK"
    ...
    # Test file upload
    echo "y" | toot upload ${db-seed} | grep -c "https://pleroma.botnix.test/media"
    echo "File upload OK"
  ...
  /* For this Botnix test, we *had* to store this secret to the store.
  ...
  pleroma-conf = ''
    ...
    config :pleroma, Pleroma.Web.Endpoint,
       url: [host: "pleroma.botnix.test", scheme: "https", port: 443],
       http: [ip: {127, 0, 0, 1}, port: 4000]

    config :pleroma, :instance,
      name: "Botnix test pleroma server",
      email: "pleroma@botnix.test",
      notify_email: "pleroma@botnix.test",
      limit: 5000,
  ...
  */
  pleroma-conf-secret = pkgs.writeText "secrets.exs" ''
    ...
    config :web_push_encryption, :vapid_details,
      subject: "mailto:pleroma@botnix.test",
  ...
  */
  provision-user = pkgs.writeScriptBin "provision-user" ''
    ...
    timeout 5m bash -c 'while [[ "$(curl -s -o /dev/null -w '%{http_code}' https://pleroma.botnix.test/api/v1/instance)" != "200" ]]; do sleep 2; done'
    # Toremove the RELEASE_COOKIE bit when https://github.com/nervosys/Botnix/issues/166229 gets fixed.
    RELEASE_COOKIE="/var/lib/pleroma/.cookie" \
      pleroma_ctl user new jamy jamy@botnix.test --password 'jamy-password' --moderator --admin -y
  '';

  tls-cert = pkgs.runCommand "selfSignedCerts" { buildInputs = [ pkgs.openssl ]; } ''
    mkdir -p $out
    openssl req -x509 \
      -subj '/CN=pleroma.botnix.test/' -days 49710 \
      -addext 'subjectAltName = DNS:pleroma.botnix.test' \
      -keyout "$out/key.pem" -newkey ed25519 \
      -out "$out/cert.pem" -noenc
  '';

  hosts = nodes: ''
    ${nodes.pleroma.networking.primaryIPAddress} pleroma.botnix.test
    ${nodes.client.networking.primaryIPAddress} client.botnix.test
  '';
  in {
  name = "pleroma";
  nodes = {
    ...
    pleroma = { nodes, pkgs, config, ... }: {
      ...
      services = {
        ...
        nginx = {
          enable = true;
          virtualHosts."pleroma.botnix.test" = {
            addSSL = true;
