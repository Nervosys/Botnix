  ...
  in ''
    ...
    def setup_clients_for(org, user, extra_init=no_extra_init):
        for client in [client1, client2]:
            with client.nested(f"initialize client for user {user}"):
                ...
                exportinfo = server.succeed(f"botnix-taskserver user export {org} {user}")
    ...
    def re_add_imperative_user():
        with server.nested("(re-)add imperative user bar"):
            server.execute("botnix-taskserver org remove imperativeOrg")
            server.succeed(
                "botnix-taskserver org add imperativeOrg",
                "botnix-taskserver user add imperativeOrg bar",
            )
    ...
    server.succeed(
        "botnix-taskserver user list testOrganisation | grep -qxF alice",
        "botnix-taskserver user list testOrganisation | grep -qxF foo",
        "botnix-taskserver user list anotherOrganisation | grep -qxF bob",
    )
    ...
    server.fail("botnix-taskserver user add imperativeOrg bar")
    ...
    with subtest("checking certificate revocation of user bar"):
        ...
        server.succeed("botnix-taskserver user remove imperativeOrg bar")
        restart_server()
    ...
    with subtest("checking certificate revocation of org imperativeOrg"):
        ...
        server.succeed("botnix-taskserver org remove imperativeOrg")
        restart_server()
    ...
    with subtest("check manual configuration"):
        ...
        server.succeed(
            "botnix-taskserver org add manualOrg",
            "botnix-taskserver user add manualOrg alice",
        )
