...
import ./make-test-python.nix ({ pkgs, lib, ... }: let
  ...
  in pkgs.runCommand "curl-impersonate-test-certs" {
    ...
    openssl req -newkey rsa:4096 -keyout ca-key.pem -out ca-csr.pem -nodes -subj '/CN=curl-impersonate-ca.botnix.test'
    openssl x509 -req -sha512 -in ca-csr.pem -key ca-key.pem -out ca.pem -extfile ${ca-cert-conf} -days 36500
    openssl x509 -in ca.pem -text

    # create server certificate and key
    openssl req -newkey rsa:4096 -keyout key.pem -out csr.pem -nodes -subj '/CN=curl-impersonate.botnix.test'
  ...
  nodes = {
    web = { nodes, pkgs, lib, config, ... }: {
      ...
      services = {
        nginx = {
          enable = true;
          virtualHosts."curl-impersonate.botnix.test" = {
            default = true;
            addSSL = true;
