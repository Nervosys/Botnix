...
in makeTest {
  ...
  testScript = ''
    ...
    # we should be able to log in as the root user, as well as the created botnix user
    unnamed.succeed(
    ...
    unnamed.succeed(
        "timeout 10 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o IdentityFile=~/.ssh/id_snakeoil botnix@localhost 'true'"
    )

    # test changing hostname via cloud-init worked
    assert (
        unnamed.succeed(
            "timeout 10 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o IdentityFile=~/.ssh/id_snakeoil botnix@localhost 'hostname'"
        ).strip()
        == "test"
    )
