...
in
{
  ...
  testScript = ''
    ...
    with subtest("virtual-machine image can be imported"):
        machine.succeed("incus image import ${vm-image-metadata}/*/*.tar.xz ${vm-image-disk}/botnix.qcow2 --alias botnix")

    with subtest("virtual-machine can be launched and become available"):
        machine.succeed("incus launch botnix ${instance-name} --vm --config limits.memory=512MB --config security.secureboot=false")
