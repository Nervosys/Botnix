...
in
{
  ...
  testScript = ''
    ...
    with subtest("Container image can be imported"):
        machine.succeed("incus image import ${container-image-metadata}/*/*.tar.xz ${container-image-rootfs}/*/*.tar.xz --alias botnix")

    with subtest("Container can be launched and managed"):
        machine.succeed("incus launch botnix container")
        with machine.nested("Waiting for instance to start and be usable"):
          retry(instance_is_up)
    ...
    with subtest("lxc-container generator configures nested container"):
        ...
        machine.succeed("incus launch botnix container --config security.nesting=true")
        with machine.nested("Waiting for instance to start and be usable"):
          retry(instance_is_up)
    ...
    with subtest("lxc-container generator configures privileged container"):
        machine.execute("incus delete --force container")
        machine.succeed("incus launch botnix container --config security.privileged=true")
        with machine.nested("Waiting for instance to start and be usable"):
          retry(instance_is_up)
