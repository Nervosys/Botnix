import ./make-test-python.nix ({ pkgs, lib, ... }: {
  ...
  testScript = ''
      ...
      assert "tmpfs" in machine.succeed("botnix-container list")

      with subtest("tmpfs container is up"):
          assert "up" in machine.succeed("botnix-container status tmpfs")


      def tmpfs_cmd(command):
          return f"botnix-container run tmpfs -- {command} 2>/dev/null"
      ...
      with subtest(
          ...
          machine.succeed(
              ...
              "test -e /var/lib/botnix-containers/tmpfs/root/test.file",
          )
      ...
      with subtest(
          ...
          machine.fail("test -e /var/lib/botnix-containers/tmpfs/some/random/path/test.file")
      ...
      with subtest(
          ...
          machine.succeed(
              "touch /var/lib/botnix-containers/tmpfs/var/test.file",
              "test -e /var/lib/botnix-containers/tmpfs/var/test.file",
              "ls -l /var/lib/botnix-containers/tmpfs/var/ | grep -q test.file 2>/dev/null",
          )
