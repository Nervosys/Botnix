import ./make-test-python.nix ({ pkgs, lib, ... }: {
  ...
  testScript = ''
    ...
    with subtest("Simple physical interface is up"):
        server.succeed("botnix-container start server")
        ...
        server.succeed("botnix-container run server -- ip a show dev eth1 >&2")

    with subtest("Physical device in bridge in container can ping server"):
        bridged.wait_for_unit("default.target")
        bridged.succeed("botnix-container start bridged")
    ...
    with subtest("Physical device in bond in bridge in container can ping server"):
        ...
        bridgedbond.succeed(
            ...
            "botnix-container run bridgedbond -- ping -w 10 -c 1 -n 10.10.0.1",
        )
  '';
})
