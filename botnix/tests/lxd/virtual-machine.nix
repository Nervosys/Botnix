...
in {
  ...
  testScript = ''
    ...
    with subtest("virtual-machine image can be imported"):
        machine.succeed("lxc image import ${lxd-image-metadata}/*/*.tar.xz ${lxd-image-disk}/botnix.qcow2 --alias botnix")

    with subtest("virtual-machine can be launched and become available"):
        machine.succeed("lxc launch botnix ${instance-name} --vm --config limits.memory=512MB --config security.secureboot=false")
