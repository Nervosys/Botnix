...
in {
  ...
  nodes.machine =
    ...
    { swapDevices = lib.mkOverride 0
      ...
      documentation.botnix.enable = lib.mkOverride 0 true;
  ...
  testScript =
    ''
      ...
      with subtest("botnix-version"):
          machine.succeed("[ `botnix-version | wc -w` = 2 ]")

      with subtest("botnix-rebuild"):
          assert "Botnix module" in machine.succeed("botnix-rebuild --help")
      ...
      with subtest("nix-db"):
          machine.succeed("nix-store -qR /run/current-system | grep botnix-")
