/*
  ...
  Based in part on botnix/tests/pleroma.
...
let
  ...
  provisionUser = pkgs.writers.writeBashBin "provisionUser" ''
    ...
    pleroma_ctl user new jamy jamy@botnix.test --password '${userPassword}' --moderator --admin -y
  '';
  ...
  tlsCert = pkgs.runCommand "selfSignedCerts" {
    ...
    mkdir -p $out
    openssl req -x509 \
      -subj '/CN=akkoma.botnix.test/' -days 49710 \
      -addext 'subjectAltName = DNS:akkoma.botnix.test' \
  ...
  sendToot = pkgs.writers.writeBashBin "sendToot" ''
    ...
    echo '${userPassword}' | ${pkgs.toot}/bin/toot login_cli -i "akkoma.botnix.test" -e "jamy@botnix.test"
    ...
    echo "y" | ${pkgs.toot}/bin/toot upload <(dd if=/dev/zero bs=1024 count=1024 status=none) \
      | grep -F -q "https://akkoma.botnix.test/media"
  '';
  ...
  checkFe = pkgs.writers.writeBashBin "checkFe" ''
    ...
    for path in "''${paths[@]}"; do
      diff \
        <(${pkgs.curl}/bin/curl -f -S -s -o /dev/null -w '%{response_code}' "https://akkoma.botnix.test$path") \
        <(echo -n 200)
    done
  '';

  hosts = nodes: ''
    ${nodes.akkoma.networking.primaryIPAddress} akkoma.botnix.test
    ...
    akkoma = { nodes, pkgs, config, ... }: {
      ...
      services.akkoma = {
        ...
        config = {
          ":pleroma" = {
            ":instance" = {
              name = "Botnix test Akkoma server";
              description = "Botnix test Akkoma server";
              email = "akkoma@botnix.test";
              notify_email = "akkoma@botnix.test";
