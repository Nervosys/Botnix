import ./make-test-python.nix ({ pkgs, ... }: {
  ...
  in ''
    ...
    with subtest("check if botnix install iso boots, network and autostart works"):
      virthost.succeed(
        "virt-install -n botnix --osinfo botnix-unstable --memory 1024 --graphics none --disk `find ${nixosInstallISO}/iso -type f | head -n1`,readonly=on --import --noautoconsole --autostart"
      )
      virthost.succeed("virsh domstate botnix | grep running")
      virthost.wait_until_succeeds("ping -c 1 botnix")
      virthost.succeed("virsh ${virshShutdownCmd} botnix")
      virthost.wait_until_succeeds("virsh domstate botnix | grep 'shut off'")
      virthost.shutdown()
      virthost.wait_for_unit("multi-user.target")
      virthost.wait_until_succeeds("ping -c 1 botnix")
