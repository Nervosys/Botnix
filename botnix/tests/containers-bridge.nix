...
import ./make-test-python.nix ({ pkgs, lib, ... }: {
  ...
  testScript = ''
    ...
    with subtest("Start the webserver container"):
        assert "up" in machine.succeed("botnix-container status webserver")

    with subtest("Bridges exist inside containers"):
        machine.succeed(
            "botnix-container run webserver -- ip link show eth0",
            "botnix-container run web-noip -- ip link show eth0",
        )
    ...
    with subtest(
        "botnix-container show-ip works in case of an ipv4 address "
        + "with subnetmask in CIDR notation."
    ):
        result = machine.succeed("botnix-container show-ip webserver").rstrip()
        assert result == ip
