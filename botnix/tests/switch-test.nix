...
in {
          ...
          system.activationScripts.restart-and-reload-test = {
            ...
            text = ''
              if [ "$NIXOS_ACTION" = dry-activate ]; then
                f=/run/botnix/dry-activation-restart-list
                g=/run/botnix/dry-activation-reload-list
              else
                f=/run/botnix/activation-restart-list
                g=/run/botnix/activation-reload-list
              fi
              cat <<EOF >> "$f"
  ...
  in /* python */ ''
    ...
    with subtest("continuing from an aborted switch"):
        ...
        machine.succeed("echo dbus.service > /run/botnix/start-list")
        out = switch_to_specialisation("${machine}", "modifiedSystemConf")
