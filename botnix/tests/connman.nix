  ...
  testScript =
    ''
      ...
      with subtest("Can set up a wireless access point"):
          ...
          client.wait_until_succeeds("connmanctl tether wifi on botnix-test reproducibility | grep -q 'Enabled'")
          client.wait_until_succeeds("iw wlan0 info | grep -q botnix-test")
    '';
})
