...
# towards a nameserver to allocate a name in the lan.botnix.test zone.
...
import ./make-test-python.nix ({ pkgs, lib, ...}: {
  ...
  nodes = {
    router = { config, pkgs, ... }: {
      ...
      services.kea.dhcp4 = {
        ...
        settings = {
          ...
          ddns-qualifying-suffix = "lan.botnix.test.";
        };
      };
    ...
    nameserver = { config, pkgs, ... }: {
      ...
      # Set up an authoritative nameserver, serving the `lan.botnix.test`
      ...
      # use TSIG keys.
      services.knot = let
        zone = pkgs.writeTextDir "lan.botnix.test.zone" ''
          @ SOA ns.botnix.test nox.botnix.test 0 86400 7200 3600000 172800
          @ NS nameserver
      ...
      in {
        ...
        settings = {
          ...
          zone."lan.botnix.test" = {
            file = "lan.botnix.test.zone";
            acl = [
              "dhcp_ddns"
            ];
          };
  ...
  testScript = { ... }: ''
    ...
    nameserver.wait_until_succeeds("kdig +short client.lan.botnix.test @10.0.0.2 | grep -q 10.0.0.3")
  '';
})
