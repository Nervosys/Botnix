import ./make-test-python.nix ({ pkgs, ...} : {
      ...
      text = ''
        if [ -e /test-the-activation-script ]; then
          if [ "$NIXOS_ACTION" != dry-activate ]; then
            ...
            echo restart-me.service > /run/botnix/activation-restart-list
            echo reload-me.service > /run/botnix/activation-reload-list
          else
            echo restart-me.service > /run/botnix/dry-activation-restart-list
            echo reload-me.service > /run/botnix/dry-activation-reload-list
          fi
        fi
      '';
    };
  ...
  testScript = /* python */ ''
    ...
    with subtest("dry activation"):
        ...
        machine.fail("test -f /run/botnix/dry-activation-restart-list")
        machine.fail("test -f /run/botnix/dry-activation-reload-list")
    ...
    with subtest("real activation"):
        ...
        machine.fail("test -f /run/botnix/activation-restart-list")
        machine.fail("test -f /run/botnix/activation-reload-list")
  '';
})
