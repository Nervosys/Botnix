import ./make-test-python.nix ({ pkgs, ... }:
  let
    ...
    container = {
      # We re-use the Botnix container option ...
  ...
  in
  {
    ...
    testScript = ''
      ...
      # Workaround for botnix-install
      machine.succeed("chmod o+rx /var/lib/machines");
      machine.succeed("botnix-install --root ${containerRoot} --system ${containerSystem} --no-channel-copy --no-root-passwd");
