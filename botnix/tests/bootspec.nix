...
in
{
  ...
  initrd = makeTest {
    ...
    testScript = ''
      ...
      bootspec = json.loads(machine.succeed("jq -r '.\"org.botnix.bootspec.v1\"' /run/current-system/boot.json"))
  ...
  specialisation = makeTest {
    ...
    testScript = ''
      ...
      sp_in_parent = json.loads(machine.succeed("jq -r '.\"org.botnix.specialisation.v1\".something' /run/current-system/boot.json"))
      sp_in_fs = json.loads(machine.succeed("cat /run/current-system/specialisation/something/boot.json"))

      assert sp_in_parent['org.botnix.bootspec.v1'] == sp_in_fs['org.botnix.bootspec.v1'], "Bootspecs of the same specialisation are different!"
    '';
  };
