import ./make-test-python.nix ({ pkgs, ... }: {
  name = "botnix-rebuild-target-host";
  ...
  testScript = { nodes, ... }:
    ...
    ''
      ...
      target.succeed("botnix-generate-config")
      deployer.succeed("scp alice@target:/etc/botnix/hardware-configuration.nix /root/hardware-configuration.nix")
      ...
      with subtest("Deploy to root@target"):
        deployer.succeed("botnix-rebuild switch -I botnix-config=/root/configuration-1.nix --target-host root@target &>/dev/console")
      ...
      with subtest("Deploy to alice@target with passwordless sudo"):
        deployer.succeed("botnix-rebuild switch -I botnix-config=/root/configuration-2.nix --target-host alice@target --use-remote-sudo &>/dev/console")
        target_hostname = deployer.succeed("ssh alice@target cat /etc/hostname").rstrip()
        assert target_hostname == "config-2-deployed", f"{target_hostname=}"

      with subtest("Deploy to bob@target with password based sudo"):
        deployer.succeed("passh -c 3 -C -p ${nodes.target.users.users.bob.password} -P \"\[sudo\] password\" botnix-rebuild switch -I botnix-config=/root/configuration-3.nix --target-host bob@target --use-remote-sudo &>/dev/console")
        target_hostname = deployer.succeed("ssh alice@target cat /etc/hostname").rstrip()
      ...
      with subtest("Deploy works with very long TMPDIR"):
        ...
        deployer.succeed(f"TMPDIR={tmp_dir} botnix-rebuild switch -I botnix-config=/root/configuration-1.nix --target-host root@target &>/dev/console")
    '';
})
