import ./make-test-python.nix ({ lib, pkgs, ...}: {
  ...
  testScript = ''
    ...
    # Generate CA.
    openssl("req -new -newkey rsa:4096 -nodes -x509 -days 7 -subj '/C=ZZ/ST=Cloud/L=Unspecified/O=Botnix/OU=Tests/CN=CA Certificate' -keyout cacert.key -out cacert.crt")

    # Generate and sign Server.
    openssl("req -newkey rsa:4096 -nodes -subj '/CN=server/OU=Tests/O=Botnix' -keyout server.key -out server.csr")
    ...
    openssl("req -newkey rsa:4096 -nodes -subj '/CN=client/OU=Tests/O=Botnix' -keyout client.key -out client.csr")
