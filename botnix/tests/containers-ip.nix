  ...
  testScript = { nodes, ... }: ''
    ...
    def get_ip(container):
        ...
        if container == "webserver4":
            ip = machine.succeed(f"botnix-container show-ip {container}").rstrip()
    ...
    for container in "webserver4", "webserver6":
        assert container in machine.succeed("botnix-container list")

        with subtest(f"Start container {container}"):
            machine.succeed(f"botnix-container start {container}")
        ...
        with subtest(f"Stop container {container}"):
            machine.succeed(f"botnix-container stop {container}")
            machine.fail(curl_host(ip))

        # Destroying a declarative container should fail.
        machine.fail(f"botnix-container destroy {container}")
  '';
})
