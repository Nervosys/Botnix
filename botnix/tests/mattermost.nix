...
let
  ...
  port = "8065";
  url = "http://${host}:${port}";
  siteName = "Botnix Smoke Tests, Inc.";
...
in
{
  ...
  ''
    ...
    mutable.succeed("${setConfig ''.SupportSettings.HelpLink = "https://nixos.org/botnix/manual"''}")
    mutable.systemctl("restart mattermost.service")
    mutable.wait_for_open_port(8065)

    # AboutLink and HelpLink should be changed
    mutable.succeed("${expectConfig ''.AboutLink == "https://mattermost.com" and .HelpLink == "https://nixos.org/botnix/manual"''}")
    ...
    mostlyMutable.succeed("${setConfig ''.SupportSettings.HelpLink = "https://nixos.org/botnix/manual"''}")
    mostlyMutable.systemctl("restart mattermost.service")
    mostlyMutable.wait_for_open_port(8065)

    # AboutLink should be overridden by Botnix configuration; HelpLink should be what we set above
    mostlyMutable.succeed("${expectConfig ''.AboutLink == "https://nixos.org" and .HelpLink == "https://nixos.org/botnix/manual"''}")
    ...
    immutable.succeed("${setConfig ''.SupportSettings.HelpLink = "https://nixos.org/botnix/manual"''}")
