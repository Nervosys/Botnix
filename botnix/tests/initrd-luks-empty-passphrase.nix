...
in {
  ...
  testScript = ''
    ...
    machine.succeed("bootctl set-default botnix-generation-1-specialisation-boot-luks-wrong-keyfile.conf")
    ...
    machine.succeed("bootctl set-default botnix-generation-1-specialisation-boot-luks-missing-keyfile.conf")
    machine.succeed("sync")
    machine.crash()
