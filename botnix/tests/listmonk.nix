import ./make-test-python.nix ({ lib, ... }: {
  ...
  testScript = ''
    ...
    campaign_data = json.loads(machine.succeed(
      generate_listmonk_request('POST', 'campaigns/1/test', data={'template_id': templates[0]['id'], 'subscribers': ['john@example.com'], 'name': 'Test', 'subject': 'Botnix is great', 'lists': [1], 'messenger': 'email'})
    ))
