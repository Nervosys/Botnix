import ./make-test-python.nix ({ pkgs, lib, ... }: {
  ...
  testScript = let
      ...
      '';
    in ''
      with subtest("Make sure we have a Botnix tree (required by ‘botnix-container create’)"):
          machine.succeed("PAGER=cat nix-env -qa -A botnix.hello >&2")

      id1, id2 = None, None

      with subtest("Create some containers imperatively"):
          id1 = machine.succeed("botnix-container create foo --ensure-unique-name").rstrip()
          machine.log(f"created container {id1}")

          id2 = machine.succeed("botnix-container create foo --ensure-unique-name").rstrip()
      ...
      with subtest(f"Put the root of {id2} into a bind mount"):
          machine.succeed(
              f"mv /var/lib/botnix-containers/{id2} /id2-bindmount",
              f"mount --bind /id2-bindmount /var/lib/botnix-containers/{id1}",
          )

          ip1 = machine.succeed(f"botnix-container show-ip {id1}").rstrip()
          ip2 = machine.succeed(f"botnix-container show-ip {id2}").rstrip()
          assert ip1 != ip2
      ...
      with subtest(
          ...
          for id in id1, id2:
              important_path = f"/var/lib/botnix-containers/{id}/very/important/data"
              machine.succeed(
      ...
      with subtest("Stop a container early"):
          machine.succeed(f"botnix-container stop {id1}")
          machine.succeed(f"botnix-container start {id1} >&2 &")
      ...
      with subtest("tmpfiles are present"):
          ...
          machine.succeed(
              "botnix-container create tmpfiles --config-file ${tmpfilesContainerConfig}"
          )
          machine.log("created, starting…")
          machine.succeed("botnix-container start tmpfiles")
          machine.log("done starting, investigating…")
          machine.succeed(
              "echo $(botnix-container run tmpfiles -- systemctl is-active foo.service) | grep -q active;"
          )
          machine.succeed("botnix-container destroy tmpfiles")
      ...
      with subtest("Destroy the containers"):
          for id in id1, id2:
              machine.succeed(f"botnix-container destroy {id}")
      ...
      with subtest("Ensure that the container path is gone"):
          print(machine.succeed("ls -lsa /var/lib/botnix-containers"))
          machine.succeed(f"test ! -e /var/lib/botnix-containers/{id1}")

      with subtest("Ensure that a failed container creation doesn'leave any state"):
          machine.fail(
              "botnix-container create b0rk --config-file ${brokenCfg}"
          )
          machine.succeed("test ! -e /var/lib/botnix-containers/b0rk")
    '';
})
