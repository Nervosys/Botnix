...
let
  ...
  makeConfig = { bootLoader, grubDevice, grubIdentifier, grubUseEfi
    ...
    pkgs.writeText "configuration.nix" ''
      ...
      { imports =
        ...
        # To ensure that we can rebuild the grub configuration on the botnix-rebuild
  ...
  # The test script boots a Botnix VM, installs Botnix on an empty hard
  ...
  testScriptFun = { bootLoader, createPartitions, grubDevice, grubUseEfi, grubIdentifier
    ...
    '' else ''
      ...
      with subtest("Create the Botnix configuration"):
          machine.succeed("botnix-generate-config --root /mnt")
          machine.succeed("cat /mnt/etc/botnix/hardware-configuration.nix >&2")
          machine.copy_from_host(
              ...
              "/mnt/etc/botnix/configuration.nix",
          )
          machine.copy_from_host("${pkgs.writeText "secret" "secret"}", "/mnt/etc/botnix/secret")

      ${optionalString clevisTest ''
        with subtest("Create the Clevis secret with Tang"):
             machine.systemctl("start network-online.target")
             machine.wait_for_unit("network-online.target")
             machine.succeed('echo -n password | clevis encrypt sss \'{"t": 2, "pins": {"tpm2": {}, "tang": {"url": "http://192.168.1.2"}}}\' -y > /mnt/etc/botnix/clevis-secret.jwe')''}
      ...
      with subtest("Perform the installation"):
          machine.succeed("botnix-install < /dev/null >&2")

      with subtest("Do it again to make sure it's idempotent"):
          machine.succeed("botnix-install < /dev/null >&2")

      with subtest("Check that we can build things in botnix-enter"):
          machine.succeed(
              """
              botnix-enter -- nix-build --option substitute false -E 'derivation {
                  name = "t";
                  builder = "/bin/sh";
                  args = ["-c" "echo botnix-enter build > $out"];
                  system = builtins.currentSystem;
      ...
      with subtest("Check whether the channel works"):
          machine.succeed("nix-env -iA botnix.procps >&2")
      ...
      with subtest(
          ...
          machine.succeed("su alice -l -c 'nix-env -iA botnix.procps' >&2")
      ...
      with subtest("Check whether botnix-rebuild works"):
          machine.succeed("botnix-rebuild switch >&2")

      # FIXME: Nix 2.4 broke botnix-option, someone has to fix it.
      # with subtest("Test botnix-option"):
      #     kernel_modules = machine.succeed("botnix-option boot.initrd.kernelModules")
      ...
      machine.succeed("botnix-rebuild boot >&2")
      machine.shutdown()
    ...
    + optionalString testFlakeSwitch ''
      ...
      with subtest("Configure system with flake"):
        ...
        machine.succeed("""
          ...
          mv /etc/botnix/hardware-configuration.nix /root/my-config/
          mv /etc/botnix/secret /root/my-config/
          rm /etc/botnix/configuration.nix
        """)
        machine.copy_from_host_via_shell(
          "${makeConfig {
      ...
      with subtest("Switch to flake based config"):
        machine.succeed("botnix-rebuild switch --flake /root/my-config#xyz")
  ...
  makeInstallerTest = name:
    ...
    makeTest {
      ...
      nodes = {
        ...
        # The configuration of the machine used to run "botnix-install".
        machine = { pkgs, ... }: {
          imports = [
          ...
          system.extraDependencies = with pkgs; [
            ...
            libxml2.bin
            libxslt.bin
            botnix-artwork.wallpapers.simple-dark-gray-bottom
            ntp
    ...
    makeLuksRootTest = name: luksFormatOpts: makeInstallerTest name {
      createPartitions = ''
        machine.succeed(
            ...
            "mkfs.ext3 -L botnix /dev/mapper/cryptroot",
            "mount LABEL=botnix /mnt",
 ...
 mkClevisBcachefsTest = { fallback ? false }: makeInstallerTest "clevis-bcachefs${optionalString fallback "-fallback"}" {
    ...
    '';
    extraConfig = ''
      boot.initrd.clevis.devices."/dev/vda3".secretFile = "/etc/botnix/clevis-secret.jwe";

      # We override what botnix-generate-config has generated because we do
  ...
  mkClevisLuksTest = { fallback ? false }: makeInstallerTest "clevis-luks${optionalString fallback "-fallback"}" {
    ...
    createPartitions = ''
      machine.succeed(
        ...
        "mkfs.ext3 -L botnix /dev/mapper/crypt-root",
        "mount LABEL=botnix /mnt",
        "mkfs.ext3 -L boot /dev/vda1",
        "mkdir -p /mnt/boot",
        "mount LABEL=boot /mnt/boot",
    ...
    '';
    extraConfig = ''
      boot.initrd.clevis.devices."crypt-root".secretFile = "/etc/botnix/clevis-secret.jwe";
    '';
    preBootCommands = ''
      tpm = Tpm()
      tpm.check()
  ...
  mkClevisZfsTest = { fallback ? false }: makeInstallerTest "clevis-zfs${optionalString fallback "-fallback"}" {
    ...
    '';
    extraConfig = ''
      boot.initrd.clevis.devices."rpool/root".secretFile = "/etc/botnix/clevis-secret.jwe";
  ...
  lvm = makeInstallerTest "lvm" {
    createPartitions = ''
      machine.succeed(
          ...
          "lvcreate --size 6G --name botnix MyVolGroup",
          "mkswap -f /dev/MyVolGroup/swap -L swap",
          "swapon -L swap",
          "mkfs.xfs -L botnix /dev/MyVolGroup/botnix",
          "mount LABEL=botnix /mnt",
      )
    '';
  };
  ...
  fullDiskEncryption = makeInstallerTest "fullDiskEncryption" {
    createPartitions = ''
      machine.succeed(
          ...
          "lvcreate -l '100%FREE' -n botnix crypt",
          "mkfs.vfat -n efi /dev/vda1",
          "mkfs.ext4 -L botnix /dev/crypt/botnix",
          "mkswap -L swap /dev/crypt/swap",
          "mount LABEL=botnix /mnt",
          "mkdir -p /mnt/{etc/botnix,boot/efi}",
          "mount LABEL=efi /mnt/boot/efi",
          "swapon -L swap",
          "mv luks.key /mnt/etc/botnix/"
      )
    '';
    bootLoader = "grub";
    grubUseEfi = true;
  ...
  bcache = makeInstallerTest "bcache" {
    createPartitions = ''
      machine.succeed(
          ...
          "udevadm settle",
          "mkfs.ext3 -L botnix /dev/bcache0",
          "mount LABEL=botnix /mnt",
          "mkfs.ext3 -L boot /dev/vda1",
          "mkdir /mnt/boot",
          "mount LABEL=boot /mnt/boot",
  ...
  btrfsSubvols = makeInstallerTest "btrfsSubvols" {
    createPartitions = ''
      machine.succeed(
          ...
          "btrfs subvol create /mnt/botnix",
          "btrfs subvol create /mnt/botnix/default",
          "umount /mnt",
          "mount -o defaults,subvol=botnix/default LABEL=root /mnt",
          "mkdir /mnt/boot",
  ...
  btrfsSubvolDefault = makeInstallerTest "btrfsSubvolDefault" {
    createPartitions = ''
      machine.succeed(
          ...
          + "$(btrfs subvol list /mnt | grep 'botnix' | awk '{print $2}') /mnt",
          "umount /mnt",
  ...
  btrfsSubvolEscape = makeInstallerTest "btrfsSubvolEscape" {
    createPartitions = ''
      machine.succeed(
          ...
          "btrfs device scan",
          "mount LABEL=root /mnt",
          "btrfs subvol create '/mnt/botnix in space'",
          "btrfs subvol create /mnt/boot",
          "umount /mnt",
          "mount -o 'defaults,subvol=botnix in space' LABEL=root /mnt",
          "mkdir /mnt/boot",
          "mount -o defaults,subvol=boot LABEL=root /mnt/boot",
      )
    '';
  ...
  stratisRoot = makeInstallerTest "stratisRoot" {
    createPartitions = ''
      machine.succeed(
        ...
        "stratis filesystem create my-pool botnix",
        "udevadm settle",

        "mount /dev/stratis/my-pool/botnix /mnt",
        "mkdir -p /mnt/boot",
        "mount /dev/vda1 /mnt/boot"
      )
    '';
