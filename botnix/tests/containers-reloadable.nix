...
in {
  ...
  nodes = {
    ...
    client_c1 = { lib, ... }: {
      ...
      containers.test1.config = {
        ...
        services.httpd.adminAddr = "botnix@example.com";
      };
    };
    client_c2 = { lib, ... }: {
      imports = [ client_base ];
  ...
  in ''
    ...
    assert "client_base" in client.succeed("botnix-container run test1 cat /etc/check")

    with subtest("httpd is available after activating config1"):
        client.succeed(
            "${c1System}/bin/switch-to-configuration test >&2",
            "[[ $(botnix-container run test1 cat /etc/check) == client_c1 ]] >&2",
    ...
    with subtest("httpd is not available any longer after switching to config2"):
        client.succeed(
            ...
            "[[ $(botnix-container run test1 cat /etc/check) == client_c2 ]] >&2",
