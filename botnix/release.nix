...
let
  ...
  makeIso =
    ...
    hydraJob ((import lib/eval-config.nix {
      ...
      modules = makeModules module {
        isoImage.isoBaseName = "botnix-${type}";
      };
  ...
  makeSystemTarball =
    ...
    in
      tarball //
        { meta = {
            description = "Botnix system tarball for ${system} - ${stdenv.hostPlatform.linux-kernel.name}";
...
in rec {
  ...
  botnix-configuration-reference-manpage = buildFromConfig ({ ... }: { }) (config: config.system.build.manual.botnix-configuration-reference-manpage);
  ...
  # Test job for https://github.com/nervosys/Botnix/issues/121354 to test
  ...
  # Ensure that all packages used by the minimal Botnix config end up in the channel.
