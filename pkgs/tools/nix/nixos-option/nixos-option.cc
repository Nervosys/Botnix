...
struct Context {
  Context(EvalState &state, Bindings &autoArgs, Value optionsRoot,
          Value configRoot)
      : state(state),
        autoArgs(autoArgs),
...
class Out {
 public:
  ...
  explicit Out(std::ostream &ostream)
      : ostream(ostream), policy(ONE_LINE), writeSinceSep(true) {}
  Out(Out &o, const std::string &start, const std::string &end,
      LinePolicy policy);
 ...
 private:
  ...
  bool writeSinceSep;
  template <typename T>
  friend Out &operator<<(Out &o, T thing);

  friend void printValue(Context &ctx, Out &out,
                         std::variant<Value, std::exception_ptr> maybeValue,
                         const std::string &path);
};

template <typename T>
Out &operator<<(Out &o, T thing) {
...
Out::Out(Out &o, const std::string &start, const std::string &end,
         LinePolicy policy)
    : ostream(o.ostream),
      indentation(policy == ONE_LINE ? o.indentation : o.indentation + "  "),
      end(policy == ONE_LINE ? end : o.indentation + end),
      policy(policy),
      writeSinceSep(true) {
  o << start;
  *this << Out::sep;
...
const std::string appendPath(const std::string &prefix,
...
bool forbiddenRecursionName(const nix::Symbol symbol,
                            const nix::SymbolTable &symbolTable) {
  // note: this is created from a pointer
  // According to standard, it may never point to null, and hence attempts to
  // check against nullptr are not allowed. However, at the time of writing, I
  // am not certain about the full implications of the omission of a nullptr
  // check here.
...
void recurse(
    const std::function<bool(const std::string &path,
                             std::variant<Value, std::exception_ptr>)> &f,
  ...
  for (const auto &child :
       evaluated_value.attrs->lexicographicOrder(ctx.state.symbols)) {
    if (forbiddenRecursionName(child->name, ctx.state.symbols)) {
      continue;
    }
    recurse(f, ctx, *child->value,
            appendPath(path, ctx.state.symbols[child->name]));
  }
}
...
Value getSubOptions(Context &ctx, Value &option) {
  Value getSubOptions = evaluateValue(
      ctx,
      *findAlongAttrPath(ctx.state, "type.getSubOptions", ctx.autoArgs, option)
           .first);
...
FindAlongOptionPathRet findAlongOptionPath(Context &ctx,
                                           const std::string &path) {
  ...
  for (auto i = tokens.begin(); i != tokens.end(); i++) {
    ...
    try {
      ...
      if (isOption(ctx, v) && isAggregateOptionType(ctx, v)) {
        ...
        v = subOptions;
        // Note that we've consumed attr, but didn't actually use it.  This is
        // the path component that's looked up in the list or attribute set that
        // doesn't name an option -- the "root" in "users.users.root.name".
      } else if (v.type() != nAttrs) {
        throw OptionPathError("Value is %s while a set was expected",
                              showType(v));
      } else {
...
// Note that "the option described by `path`" is not trivial -- if path
// describes a value inside an aggregate option (such as users.users.root), the
// *option* described by that path is one path component shorter (eg:
// users.users), which results in f being called on sibling-paths (eg:
// users.users.nixbld1).  If f doesn't want these, it must do its own filtering.
void mapOptions(const std::function<void(const std::string &path)> &f,
                Context &ctx, const std::string &path) {
  auto root = findAlongOptionPath(ctx, path);
  recurse(
      [f, &ctx](const std::string &path,
                std::variant<Value, std::exception_ptr> v) {
        bool isOpt = std::holds_alternative<std::exception_ptr>(v) ||
...
void mapConfigValuesInOption(
    const std::function<void(const std::string &path,
                             std::variant<Value, std::exception_ptr> v)> &f,
    const std::string &path, Context &ctx) {
  Value *option;
  try {
    option =
        findAlongAttrPath(ctx.state, path, ctx.autoArgs, ctx.configRoot).first;
  } catch (Error &) {
  ...
  recurse(
      [f, ctx](const std::string &path,
               std::variant<Value, std::exception_ptr> v) {
        bool leaf = std::holds_alternative<std::exception_ptr>(v) ||
                    std::get<Value>(v).type() != nAttrs ||
...
void describeDerivation(Context &ctx, Out &out, Value v) {
  ...
  try {
    Path drvPath = i != v.attrs->end()
                       ? ctx.state.coerceToPath(
                             i->pos, *i->value, pathset,
...
void printValue(Context &ctx, Out &out,
                std::variant<Value, std::exception_ptr> maybeValue,
                const std::string &path);
...
void printValue(Context &ctx, Out &out,
  ...
  try {
    ...
    if (e.msg() == "The option `" + path + "' is used but not defined.") {
      ...
      // botnix-option, we are looking at this value only to print it.
...
void printConfigValue(Context &ctx, Out &out, const std::string &path,
                      std::variant<Value, std::exception_ptr> v) {
  out << path << " = ";
...
bool starts_with(const std::string &s, const std::string &prefix) {
  return s.size() >= prefix.size() &&
         std::equal(s.begin(), std::next(s.begin(), prefix.size()),
  ...
  mapOptions(
      [&ctx, &out, &path](const std::string &optionPath) {
        mapConfigValuesInOption(
            [&ctx, &out, &path](const std::string &configPath,
                                std::variant<Value, std::exception_ptr> v) {
...
void printAttr(Context &ctx, Out &out, const std::string &path, Value &root) {
  try {
    printValue(ctx, out,
               *findAlongAttrPath(ctx.state, path, ctx.autoArgs, root).first,
               path);
  } catch (Error &e) {
    out << describeError(e);
  }
...
void printOption(Context &ctx, Out &out, const std::string &path,
                 Value &option) {
  out << "Value:\n";
...
int main(int argc, char **argv) {
  ...
  MyArgs myArgs(std::string(nix::baseNameOf(argv[0])),
                [&](Strings::iterator &arg, const Strings::iterator &end) {
                  if (*arg == "--help") {
                    nix::showManPage("botnix-option");
                  } else if (*arg == "--version") {
                    nix::printVersion("botnix-option");
