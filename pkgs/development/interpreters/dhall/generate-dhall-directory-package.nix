{ dhall-botpkgs, lib, stdenv }:

# This function calls `dhall-to-botpkgs directory --fixed-output-derivations`
# within a Nix derivation.
#
# This is possible because
# `dhall-to-botpkgs directory --fixed-output-derivations` will turn remote
...
lib.makePackageOverridable
  ( { src
    ...
    stdenv.mkDerivation {
      ...
      buildCommand = ''
        dhall-to-botpkgs directory --fixed-output-derivations --file "${file}" "${src}" ${lib.optionalString document "--document"} > $out
      '';

      nativeBuildInputs = [ dhall-botpkgs ];
    }
  )
