 ...
 * in combination with the syslibroot option. In botpkgs, the .tbd files refer
 ...
 * result.
 *
 * In botpkgs each framework is a distinct store path and some frameworks
...
static yaml_node_t *get_mapping_entry(yaml_document_t *document,
                                      yaml_node_t *mapping, const char *name) {
  if (!mapping) {
  ...
  for (yaml_node_pair_t *pair = mapping->data.mapping.pairs.start;
    ...
    if (strncmp((const char *)key->data.scalar.value, name,
...
static int emit_reexports_v2(yaml_document_t *document) {
  ...
  for (yaml_node_item_t *export = exports->data.sequence.items.start;
       export < exports->data.sequence.items.top; ++export) {
    yaml_node_t *export_node = yaml_document_get_node(document, *export);

    yaml_node_t *reexports =
        get_mapping_entry(document, export_node, "re-exports");
    ...
    for (yaml_node_item_t *reexport = reexports->data.sequence.items.start;
         reexport < reexports->data.sequence.items.top; ++reexport) {
...
static int emit_reexports_v4(yaml_document_t *document) {
  yaml_node_t *root = yaml_document_get_root_node(document);
  yaml_node_t *reexports =
      get_mapping_entry(document, root, "reexported-libraries");
  ...
  for (yaml_node_item_t *entry = reexports->data.sequence.items.start;
       entry < reexports->data.sequence.items.top; ++entry) {
    ...
    for (yaml_node_item_t *lib = libs->data.sequence.items.start;
         lib < libs->data.sequence.items.top; ++lib) {
