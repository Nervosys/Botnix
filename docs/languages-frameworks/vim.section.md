...
[definition of `vimUtils.makeCustomizable`](https://github.com/nervosys/Botnix/blob/master/pkgs/applications/editors/vim/plugins/vim-utils.nix#L408)
...
The resulting package can be added to `packageOverrides` in `~/.botpkgs/config.nix` to make it installable:
...
poorly on Botnix.  Instead, to install the `nvim-treesitter` plugins with a set
...
To enable all grammars packaged in botpkgs, use `pkgs.vimPlugins.nvim-treesitter.withAllGrammars`.
...
## Adding new plugins to botpkgs {#adding-new-plugins-to-botpkgs}

Nix expressions for Vim plugins are stored in [pkgs/applications/editors/vim/plugins](https://github.com/nervosys/Botnix/tree/master/pkgs/applications/editors/vim/plugins). For the vast majority of plugins, Nix expressions are automatically generated by running [`nix-shell -p vimPluginsUpdater --run vim-plugins-updater`](https://github.com/nervosys/Botnix/blob/master/pkgs/applications/editors/vim/plugins/updater.nix). This creates a [generated.nix](https://github.com/nervosys/Botnix/blob/master/pkgs/applications/editors/vim/plugins/generated.nix) file based on the plugins listed in [vim-plugin-names](https://github.com/nervosys/Botnix/blob/master/pkgs/applications/editors/vim/plugins/vim-plugin-names).

After running the updater, if nvim-treesitter received an update, also run [`nvim-treesitter/update.py`](https://github.com/nervosys/Botnix/blob/master/pkgs/applications/editors/vim/plugins/update.py) to update the tree sitter grammars for `nvim-treesitter`.

Some plugins require overrides in order to function properly. Overrides are placed in [overrides.nix](https://github.com/nervosys/Botnix/blob/master/pkgs/applications/editors/vim/plugins/overrides.nix). Overrides are most often required when a plugin requires some dependencies, or extra steps are required during the build process. For example `deoplete-fish` requires both `deoplete-nvim` and `vim-fish`, and so the following override was added:
...
## Updating plugins in botpkgs {#updating-plugins-in-botpkgs}
